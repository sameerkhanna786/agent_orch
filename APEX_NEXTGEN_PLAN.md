...
*A synthesis plan fusing APEX v1, the state of the art, and a proposed redesign — built on the orchestration-as-code pattern that Claude Code's dynamic-workflows feature popularized — re-implemented by APEX as its own vendor-neutral engine, so it runs on Codex, Claude Code, or both. ("ultracode" denotes only the Claude-Code setting, not the paradigm; see §24.7.) Every adopted mechanism was checked against adversarial verdicts and published evidence.*
...
- [24. Critical-Review Response & Revisions](#24-critical-review-response-revisions)

## Executive Summary

**APEX-Ω is a vendor-neutral dynamic-workflow ENGINE.** It re-implements the orchestration-as-code pattern that Claude Code's dynamic-workflows feature popularized (a JavaScript script orchestrates isolated subagents, keeps intermediate state in script variables rather than the conversation, and converges via independent verify/refute — [code.claude.com/docs/en/workflows](https://code.claude.com/docs/en/workflows)) as APEX's own engine, and **ADDS** five APEX-defined primitives (`agent`/`parallel`/`pipeline`/`phase`/`budget`), deterministic journaled replay, a WAL, and cross-vendor leaf execution — none of which are documented Claude guarantees — whose leaf workers are *any* coding CLI/API — Codex (`codex_cli`), Claude Code (`claude_cli`), Gemini, opencode, or **mixed in one run**. The hardened **APEX v1 substrate** is preserved verbatim as the inner kernel; the proposed-redesign ("v3") ideas are admitted only as **workflow patterns over that substrate**, each judged against the adversarial verdicts and accepted, modified, deferred, or rejected — never rubber-stamped.
...
APEX v1 already solved selection. Its **Cardinal Safety Contract** (`APEX_DESIGN_BLUEPRINT.md` §13.1) states: *execution evidence is authoritative; soft/LLM signals may only re-rank within an execution-verified tier or downgrade an already-accepted candidate, never promote an unverified one.* It is enforced structurally — `_apply_evidence_bound_review` flips `accepted` only `True → False`, and the upstream execution-accepted filter (`_prefer_accepted_clusters`) restricts candidates to execution-accepted clusters before ranking, so soft signals only reorder within the already-accepted set (see §24.7.3 for the accurate two-part contract). This is the direct mitigation of the false-positive failure mode the resampling literature proves is fatal, and it is what makes APEX's number trustworthy where a pure LLM-judge pipeline is not.
...
- **`pipeline()` per-item streaming.** No barrier between reproduce → localize → patch → verify stages; item A can be patching while item B is still reproducing. Wall-clock collapses from sum-of-slowest-per-stage to slowest-single-chain (Section 2, 16). `pipeline()` is one of four net-new builds and the largest engineering risk: it carries seven contracts (stage identity/idempotence, cancellation/partial artifacts, backpressure, per-stage budgets, stage-level promotion, cleanup/quarantine) and ships behind a scaffolded/local feature flag before any provider-backed/live/V5/frontier use (see §24.5 for the seven contracts and the staged build order). v1 has only barrier waves.
...
APEX-Ω **re-implements, as its own vendor-neutral engine,** the orchestration-as-code pattern Claude Code's dynamic-workflows feature popularized ([code.claude.com/docs/en/workflows](https://code.claude.com/docs/en/workflows)): a program — not a conversation — holds the plan, fans scoped work out to isolated coding **workers** (Codex, Claude Code, or any agent CLI/API, even mixed in one run), keeps every intermediate result in **script variables and a durable journal rather than a chat window**, and converges via execution-grounded **verify-and-refute**. The five primitives below are APEX-defined, not documented Claude APIs (see §24.7). This section specifies that substrate: the orchestrator–worker model, the five primitives with exact semantics, the determinism/journaling discipline that makes restart-survivable resume possible, and the precise places where APEX v1 has already converged on the paradigm versus where it falls short and must be extended.
...
APEX re-implements five vendor-neutral primitives. None of these is a documented Claude workflow API — the Claude doc defines a workflow only as "a JavaScript script that orchestrates subagents" and names no primitive API; `pipeline()` in particular has no Claude analog. These are APEX's own engine surface (see §24.7). v1 already implements the first two and analogs of three more; the engine consolidates them so the *orchestration program*, not bespoke Python in `solver.py`, composes them.
...
2. **"Substrate exceeds the base model" vs. the Cardinal Safety Contract.** The engine must **not** gain the power to promote an unverified candidate. In the new engine, selection/acceptance primitives keep **soft signals downgrade-only**, so a soft/LLM signal can never convert an execution-invalid candidate into an accepted patch (v1’s `_apply_evidence_bound_review` flips `accepted` only `True → False`; the accurate two-part contract is §24.7.3). This is the rule that converts best-of-N into trustworthy gains and counters the false-positive inversion that reward-hacking makes worse as capability scales ([ImpossibleBench: GPT-5 exploits tests 76%, arXiv:2510.20270](https://arxiv.org/html/2510.20270v1)). It is an **engine-level invariant**, carried verbatim into Section 13.
...
APEX-Ω is not a rewrite; it is a staged extraction from a working but **actively-hardening** kernel. The kernel's **invariants** (Section 4.2) are sound and reused as-is, but its runtime **stability** is gated by the Phase -1 stabilization suite (§24.6) before any extraction begins; the current open residual is narrow (~6 orchestrator coverage-gap regressions plus a stale `lastfailed` cache whose other collectable entries pass), not broad multi-subsystem failure (see §24.5 for staged extraction, §24.6 for stabilization). This section is the load-bearing recap of that kernel — APEX v1, the "Adaptive Parallel EXecution" orchestrator — and it serves three jobs simultaneously. First, it fixes the **substrate**: the exact contract, control flow, and invariants the redesign builds *on top of* and is forbidden to weaken (see Section 7 for how these become first principles, and Section 18 for the kept/modified/dropped ledger). Second, it inventories the **reusable assets** so later sections cite concrete code seams rather than reinvent them (the engine in Section 2/8, verification in Section 13, the controller in Section 14, the model economy in Section 12). Third, it names the **ceiling** — the four change-seams and the cost stack — that every redesign mechanism is justified against.
...
- The selection safety property is the upstream execution-accepted filter (`_prefer_accepted_clusters`): candidates are restricted to execution-accepted clusters before ranking, so the soft-led ranking tuple only reorders within the already-accepted set and never grants acceptance (see §24.7.3). The proposed engine additionally hardens this with a fixed lexicographic key (content-derived final tiebreak, never insertion order):
...
The prior-art landscape decomposes into two axes, and the gap APEX-Ω stakes is their unoccupied *product*, not either axis alone. **Axis A — no-retrain held-out routing** is already owned: Router-R1 ([2506.09033](https://arxiv.org/abs/2506.09033)) does descriptor-conditioned routing to unseen models without retraining, and RouteLLM ([2406.18665](https://arxiv.org/abs/2406.18665)) transfers when strong/weak models are swapped at test time — but **only on QA, with no execution-authoritative patch acceptance.** **Axis B — learned budget-aware/structure-guided orchestration over a worker pool** is also owned: Puppeteer ([2505.19591](https://arxiv.org/abs/2505.19591)) optimizes over a fixed Titan pool; MACA ([2605.25746](https://arxiv.org/abs/2605.25746), a GraphSpec structural prior + GRPO policy) does structure-guided budget-aware orchestration; AgentConductor ([2602.17100](https://arxiv.org/html/2602.17100)) and AFlow ([2410.10762](https://arxiv.org/abs/2410.10762)) train/test on a **fixed worker pool with no execution-authoritative patch gate.** No single paper occupies the product A x B — open-pool, no-retrain, held-out-vendor routing **under execution-authoritative repo-level SWE patch acceptance** — which is the unclaimed gap, the realistic deployment of a vendor-neutral engine where the worker set is *open* — new (vendor, model) pairs appear, old ones are deprecated or rate-limited mid-run, and the controller has never seen a given worker's capability/cost profile. This is the defensible contribution APEX-Ω stakes (Section 14, Section 19): an **open-pool active controller via learned capability/cost profiles** that can route to a worker it has never trained on by learning a transferable profile (difficulty-conditioned success rate, cost, latency, failure modes) rather than memorizing a fixed pool's indices.
...
The redesign is **not a rewrite**. The adversarial verdict is blunt that v1 "already converged on the orchestrator-worker model… so the redesign is a **staged extraction** of working code: v1's L2 behaviors are real and reused but entangled inside `solver.py`'s serial stages and `rollout/engine.py`'s scheduler, not exposed behind the named abstractions. The net-new builds are **four** — `pipeline()`, the node-level `(agent, stage, item)` workflow journal, the normalized Executor abstraction, and `CapabilityProfile`/`negotiate()` — plus artifact replay and learned per-decision routing; everything else is behavior reuse via extraction behind compatibility adapters, with the old path retained as fallback until parity is proven — bounding execution risk." The architecture therefore maps each amplifier onto an *existing* v1 seam and constrains it to feed L2, never to replace it.
...
The CTDG **feeds priors; the verifier decides** — where "the verifier decides" is the corrected invariant of Section 13 (the execution acceptance bar plus the upstream accepted-filter, **not** the ranking-tuple order). It is never treated as an oracle and is never on the acceptance path. **Build order is mandated:** CTDG ships first as telemetry + test prioritization + canary metrics with a full-suite backstop; the dynamic-prune channel is a later, per-repo-gated milestone that enters the engine only **after** the v1 stabilization slice (§24.6, §24.7).
...
This section preserves APEX v1's verification kernel **verbatim** as the **actively-hardening verification substrate** (caveat: the substrate has dead/unexercised wiring that Phase -1 gate G9 must fix before these paths are relied upon — the C3-defer, `mock_path`/`attribute_chain` validators, and the non-blocking focal-symbol fabrication gate; V4 #2/#3/#4 — see §24.6, §24.7), then layers exactly two evidence-supported extensions on top — a swappable generative critic *for discrimination only*, and a single re-usable `verify_and_refute()` primitive callable at multiple granularities — both bound strictly by the Cardinal Safety Contract.
...
Enforced structurally (v1 §13.1): `_apply_evidence_bound_review` flips `accepted` only `True -> False`; `VerificationResult` has no `passed`/`status` field (`accepted` is the gate); the safety property is **not** a tuple-ordering guarantee (in fact `combined_score`, a soft blend of public/critic/process/evidence/consensus signals, is the *leading* tuple key, ranking *above* `accepted` and `verification_score`). It is a two-part contract verified against `selector.py`: **(1)** acceptance is computed **only** by the execution-grounded `_verification_meets_acceptance_bar`; `_apply_evidence_bound_review` flips `accepted` only `True -> False` and is fail-open; **(2)** execution-acceptance filtering happens **upstream** via `_prefer_accepted_clusters` (`return accepted or policy_valid`), so the `combined_score`-led ranking tuple only orders candidates **within** the already-accepted set (and ranks non-accepted candidates only when zero accepted exist). Stated plainly: soft/learned signals may allocate budget, rank unaccepted candidates for more work, or break ties among execution-valid candidates, but can **never** convert an execution-invalid candidate into an accepted patch. `VerificationResult` still has no `passed`/`status` field (`accepted` is the gate), abstention is first-class, and `-cluster_id` is the final tiebreak that kills slot-0 bias (never insertion order). Pre-execution scorers (Sections 9–10) set priors/budget only (see §24.7).
...
> **Hard rule:** the critic never promotes an unverified candidate — `hybrid_select` scores only the already-verified `tier`, so the critic only breaks ties among execution-valid candidates (see §24.7.3).
...
1. **Cardinal Contract is below the controller.** The controller may set branch priors, budget share, vendor/model, and effort. It **cannot** promote, accept, or rank-above any candidate that lacks execution evidence. Soft/learned controller signals re-rank *within* an execution-verified tier or downgrade only — never promote; enforcement is the acceptance gate (`_verification_meets_acceptance_bar`) plus the upstream accepted-filter (`_prefer_accepted_clusters`), **not** the ranking-tuple order (Section 13). The controller's authority is allocation/routing/budget; it never reaches the acceptance bar or the accepted-filter (see §24.7). A controller that could elevate an unverified patch would re-introduce exactly the inference-scaling false-positive failure mode the substrate exists to prevent.
...
| Open-pool generalization | Does a *held-out* backend get sane routing? | Per the pre-registered held-out-vendor protocol (§24.4) + its calibration-to-eval transfer plot: APEX's inference-cost-matched resolved-rate on held-out vendor C strictly dominates B4 and the AOrchestra/descriptor-routing ceilings (B6/B7), **no retraining, no vendor one-hot**, OOD diagnostics reported. A10 binds the frozen split, no-one-hot assertion, parity envelope, separate cost ledger, and OOD diagnostics; A9 is the one-hot negative control that must FAIL to route C. (§24.4) |
...
We **upgrade** v1's existing escrow-WAL durability (CCEDF: fsync-durable, flock-guarded, monotonic-seq, idempotent exactly-once) from a one-candidate confirmed-pass backstop into a node-level `(agent, stage, item)` graph journal — a **per-`agent()`-call write-ahead log** — **adding graph coverage to existing durability, not durability from scratch.** This journal powers **artifact** replay (resume/audit); v1's exact LLM/tool-call replay is a **separate, retained** product. The full per-node record (stage, prompt_hash, profile_hash, tool_permissions, verifier_status, terminal_state, parent ids, latency) is specified authoritatively in §24.7; the §2.5 sketch defers to it. The escrow WAL already proved the hard part — making concurrent, nondeterministically-*timed* operations replay-stable by ordering on a **monotonic seq (not wall-clock)** with idempotent keys so duplicate appends are harmless. We reuse that exact pattern per node.
...
test_kill_midrun_identical_resume:
  ...
  6. ASSERT (terminal dispatch set): the set of (node_id, stage, item_id) reaching
            terminal_state is identical across the kill/resume boundary.
  6b. ASSERT (accepted artifact verdict): the SELECTED WINNER has the same content-sha,
            the same re-run verifier_status / accept decision, and the same diff text.
            Exact LLM/tool-call replay retains its own verify=True check; until both this
            and the terminal-dispatch-set assertion are green, durable resume is unbuilt (see §24.7).
...
- The journal records *which* branches were expanded vs pruned, but selection still runs the full execution-authoritative cascade over whatever survivors reached it; the upstream execution-accepted filter runs before ranking, so soft signals only reorder within the accepted set (Section 13; §24.7.3).
...
Every **Adopt-modified** and every **Reject** in this ledger traces back, directly or indirectly, to one v1 invariant: the **Cardinal Safety Contract** (APEX_DESIGN_BLUEPRINT.md §13.1) — *execution evidence is authoritative; soft signals may re-rank within an execution-verified tier or downgrade an already-accepted candidate, but may NEVER promote an unverified candidate.* Enforced structurally in v1 by `_apply_evidence_bound_review` (flips `accepted` only `True→False`) and the upstream execution-accepted filter (`_prefer_accepted_clusters`) that runs before ranking, so soft signals only reorder within the accepted set (see §24.7.3). The redesign's bolder mechanisms (pruning gates, plan scoring, blackboard push) are admitted *only in forms that cannot make a soft signal load-bearing for exclusion* — because the inverse-equivalent violation (a soft signal *suppressing* a would-be-verified candidate) is just as fatal as promotion. This is why three redesign mechanisms have a gate-form **Reject** sitting beside a hint-form **Adopt-modified**.

### 18.2 The canonical ledger (grouped by origin × disposition)

The following table is the human-readable rendering of the `accepted_mechanisms` array. The `Origin` column tags each idea: **v1** (the actively-hardening substrate), **redesign** (the v3 prose blueprint), **SOTA** (the research digest), **paradigm** (the vendor-neutral dynamic-workflow model). The `Verdict / finding basis` column is the traceability required by Chief Guidance — every disposition points to either an adversarial verdict or a v1/paradigm digest finding.
...
| Learned task/budget-conditioned structural prior + RL orchestration policy | MACA ([2605.25746](https://arxiv.org/abs/2605.25746): GraphSpec structural prior + GRPO policy; +8.42% avg acc / 43.19% fewer tokens vs adaptive MA baselines) | §14 active controller; baseline/ablation **B5 / A12** | **Learned structure-guided budget-aware orchestration *is* MACA — APEX must not claim it.** MACA is fixed-pool over QA/math/function-or-competition code, **not** repo-SWE / execution-authoritative / open-pool / held-out, so it is a baseline/ablation, not the headline. |
| Dynamic cross-vendor sub-agents + cost-aware orchestration + execution-verified SWE patches | AOrchestra ([2602.03786](https://arxiv.org/abs/2602.03786)) | §14 active controller; fixed-pool ceiling baseline **B6** | Covers the vendor-mixed pool + repo-SWE + execution-authoritative acceptance surface — but over a **FIXED dev pool**: it does **not** evaluate held-out/unseen models or no-retrain cross-vendor generalization (its plug-and-play test swaps *frameworks*, not *models*). The held-out-vendor axis remains unoccupied. |
...
We say all of the above, in this order, *before* stating what is new. (Provenance note: an earlier research pass falsely concluded MACA did not exist — an abstract-only/WebSearch check missed body-only terms like GraphSpec/GRPO; MACA was confirmed real via direct `/abs/` + `/html/` fetch and is included above. See §24.2.)
...
operator/workflow nodes. AOrchestra ([2602.03786](https://arxiv.org/abs/2602.03786))
creates vendor-mixed sub-agents at runtime and accepts patches via the
execution-authoritative SWE-Bench harness, but does **not** evaluate
held-out/unseen models or no-retrain cross-vendor generalization — its pool is a
fixed dev pool, and its only plug-and-play test swaps sub-agent *frameworks*, not
*models*. Conversely, no-retrain routing to unseen models **does** exist
(Router-R1 [2506.09033](https://arxiv.org/abs/2506.09033), RouteLLM
[2406.18665](https://arxiv.org/abs/2406.18665)) — but only on QA, with no
execution gate. APEX-Ω therefore does **not** claim no-retrain held-out routing
*per se* as the invention; the contribution is its instantiation **under
execution-authoritative repo-level SWE patch acceptance** with cost-matched
budgets — the unoccupied product of the two axes (see §24.1, §24.2). APEX-Ω
...
Four further baselines pin APEX against the closest published prior art, all run under the identical Normalized Executor + standardized scaffold + agent-unreachable grader:

| ID | Baseline | What it isolates | Implementation note |
|---|---|---|---|
| B5 | MACA/GraphSpec-style structure-guided orchestration | Structure-guided budget-aware ceiling on a fixed pool. | [MACA](https://arxiv.org/abs/2605.25746) (GraphSpec prior + GRPO); report training + data cost. |
| B6 | AOrchestra-style fixed-pool dynamic sub-agent orchestration | The fixed-pool repo-SWE/execution ceiling. | [AOrchestra](https://arxiv.org/abs/2602.03786); no held-out experiment — must retrain or degrade on held-out vendor C. |
| B7 | Router-R1/DAAO/RouteLLM-style descriptor/transfer routing | The transfer-to-unseen-model ceiling. | [Router-R1](https://arxiv.org/abs/2506.09033) / [RouteLLM](https://arxiv.org/abs/2406.18665) / DAAO; same descriptor metadata as APEX (no vendor one-hot). |
| B8 | AB-MCTS/AgentConductor-style adaptive workflow search | Adaptive-search ceiling. | [AgentConductor](https://arxiv.org/html/2602.17100) / AB-MCTS. **DeLM explicitly excluded** (unverifiable). |

B0 and B4 together pin down the two confounds reviewers probe hardest: B0 says "did you beat a single frontier model at equal cost," B4 says "is your win just from mixing vendors, or from *learning* how to route them." The minimum reviewer-demanded result (Section 20.6) is dominance over **B0, B1, B3 AND B5, B6** on a cost-matched Pareto plot **plus** the held-out-vendor split (see §24.3, §24.4).
...
This section sequences the build of APEX-Ω from APEX v1's **actively-hardening substrate, beginning with a gated stabilization phase (Phase -1, §24.6),** to the full evidence-grounded, vendor-neutral dynamic-workflow orchestrator described in Sections 8–17. It is decisive about ordering, names the v1 code reused at each stage, makes every research-bearing amplifier a default-off ablation flag, and treats the held-out-vendor harness as a continuous gate rather than a closing chore. The governing fact is that **stabilization (Phase -1, §24.6) is the true critical path — the engine, the normalized Executor, and the journal cannot be soundly extracted until its gates (G1–G9 + fairness F1–F4) are green on the pinned slice**: and once stabilized, nothing downstream — not speculative search (Section 9), not the model economy (Section 12), not the learned controller (Section 14) — is meaningful or even reproducible until those three exist and survive a kill-mid-run. After Phase 0, the remaining phases are largely parallelizable, because they ride on an instrumented substrate that records every `agent()` decision (see Section 15 for the journal, Section 14 for why the controller needs it).
...
## 24. Critical-Review Response & Revisions

*This section revises the plan to address every finding and required change in `APEX_NEXTGEN_CRITICAL_REVIEW.md` (2026-06-14). It narrows the novelty claim to the single defensible two-axis intersection, adds the missing prior art and baselines (including **MACA, arXiv:2605.25746**, which an earlier research pass wrongly judged non-existent — see §24.2), replaces "preserve the v1 kernel" with a staged-extraction plan, adds a pre-implementation stabilization phase, specifies the durable journal / exact-vs-artifact replay split, restates the verification invariant accurately, and pre-registers the held-out-vendor evaluation protocol. The finding→resolution map is §24.1. Sources cited here were verified against ground truth (arXiv direct-fetch for papers, file:line for v1 source); unverifiable citations (e.g. DeLM) were dropped, not invented.*

### 24.1 Critical-Review Response Map, Narrowed Novelty, and Fallback Paper Positions

This section answers, point by point, every finding and required change in
[`APEX_NEXTGEN_CRITICAL_REVIEW.md`](APEX_NEXTGEN_CRITICAL_REVIEW.md) (dated
2026-06-14). It is written as a tightening pass, not a rewrite: it preserves the
§19 spine ("the novelty rides on a *conjunction*, not on any single mechanism")
and the Cardinal Safety Contract (§13.1) verbatim, and it limits every in-place
edit to the existing body to *removing flagged overstatements* — never adding a
new claim. The structure of §24 is: this map and the narrowed novelty thesis
(§24.1); the prior-art collision map and the MACA verification-discrepancy
record (§24.2); the revised baseline/ablation set (§24.3); the pre-registered
held-out-vendor protocol and capability-profile construction rules (§24.4); and
the remaining engineering/stabilization required-changes (§24.5+). A coding agent
can treat the response-map table below as the authoritative index of where each
review item is resolved.

#### 24.1.1 Response map (findings 1–10 and required-changes 1–10)

Every row maps a review item to the exact §24.x subsection or named in-place edit
that resolves it. "In-place edit" means a flagged-overstatement removal in the
existing §1–§23 body; all such edits are catalogued in §24.6.

| Review item | Resolves where |
|---|---|
| **Finding 1** — Missing MACA is a novelty gap | §24.2 (collision-map row for MACA, arXiv:2605.25746, with "what it does NOT cover" column) + §24.2 verification-discrepancy record + §24.3 (baseline **B5**) + ablation **A12** |
| **Finding 2** — Novelty narrower than claimed | §24.1.2 (narrowed thesis) + §24.2 (component-level collision map) |
| **Finding 3** — v1 is not a clean stable kernel | §24.5 (staged-extraction plan; v1 facts cited at file:line) |
| **Finding 4** — Test/benchmark history vs "hardened substrate" | §24.5 (stabilization gates) + in-place edit removing "hardened substrate" framing |
| **Finding 5** — Claude dynamic-workflow attribution | §24.5 (Claude-leaf containment rules) + in-place edits at Exec-Summary (line 33), intro (lines 3, 144) |
| **Finding 6** — Evaluation plan incomplete (baselines) | §24.3 (full baseline families) + §24.4 (protocol) |
| **Finding 7** — Held-out-vendor generalization underspecified | §24.4 (capability-profile construction + OOD diagnostics + logging schema) |
| **Finding 8** — Verification contract overstates tuple mechanics | §24.5 (two-part invariant: acceptance gate + ranking scope) + in-place edit to §13.1 tuple sentence + property tests |
| **Finding 9** — `pipeline()` is the largest engineering risk | §24.5 (stage-extraction-first staging, feature-flagged scaffolded pipeline) |
| **Finding 10** — Replay/resume need two products | §24.5 (exact-replay kept; artifact-replay added; kill/resume CI smoke) |
| **Req-change 1** — Add prior art to related work / baselines | §24.2 + §24.3 |
| **Req-change 2** — Rewrite novelty around the narrow intersection | §24.1.2 (verbatim narrowed thesis) |
| **Req-change 3** — Pre-implementation v1 stabilization phase | §24.5 (stabilization gates) |
| **Req-change 4** — Replace "preserve v1 kernel" with "staged extraction" | §24.5 (staged-extraction ladder) |
| **Req-change 5** — Define the workflow journal schema | §24.5 (journal field schema; extends §19.5.1 `JournalEntry`) |
| **Req-change 6** — Native Claude workflow containment explicit | §24.5 (Claude-leaf containment rules) |
| **Req-change 7** — Pre-register held-out-vendor protocol + profile rules | §24.4 |
| **Req-change 8** — Executable benchmark-fairness checks | §24.4 (fairness checks as CI gates, not prose) |
| **Req-change 9** — CTDG first as telemetry + backstop, not a gate | §24.5 (CTDG telemetry-first) — consistent with §13.1 inverse-violation symmetry and §10 |
| **Req-change 10** — Define fallback paper positions | §24.1.3 (three positions, below) |

No review item is left without a resolving location. Where a row names an
"in-place edit," the edit is a deletion or down-scoping of an overstatement, and
the accepted-mechanism inventory of §18 (Fusion Ledger) is unchanged by it.

#### 24.1.2 The narrowed novelty claim (stated verbatim)

The plan's defensible contribution is restated, verbatim, as the canonical claim
the rest of §24 keeps consistent with:

> APEX-Ω's single defensible contribution is the instantiation of **no-retrain,
> held-out-vendor search/route-policy generalization under
> execution-authoritative, repo-level SWE patch acceptance with cost-matched
> budgets.** Stated as a falsifiable thesis: a learned controller that, via
> learned capability/cost profile vectors (never one-hot vendor identity), routes
> to a vendor/model held out from training — with no controller retraining and no
> online updates on the held-out vendor's tasks — and retains
> cost-quality-Pareto dominance over the strongest single model, cost-equal
> verified best-of-N, a re-trained published learned orchestrator
> (Puppeteer/AFlow/MACA-style), an AOrchestra-style fixed-pool dynamic-subagent
> orchestrator, and a Router-R1/RouteLLM-style descriptor-routing baseline, while
> every accept stays gated exclusively by the execution-grounded acceptance bar.

This is framed explicitly as the **product of two axes** — the cell no single
paper occupies:

```text
AXIS-A  (no-retrain held-out routing via descriptors/profiles)
        owned, for QA, by Router-R1 (2506.09033) and RouteLLM (2406.18665)
  ×
AXIS-B  (repo-level SWE + execution-authoritative patch acceptance + vendor-mixed pool)
        owned, over a FIXED pool, by AOrchestra (2602.03786)
  =
APEX-Ω's claim: the unoccupied PRODUCT, evaluated under a pre-registered
        held-out-vendor protocol with separate training/calibration/inference
        cost accounting (§24.4).
```

Two concessions are made first, in the §19.1 disclaimer spirit:

- **No-retrain held-out routing per se is NOT claimed.** Router-R1 already does
  descriptor-conditioned no-retrain routing to unseen models (e.g.
  Palmyra-Creative-122B) and RouteLLM transfers when the strong/weak pair is
  swapped at test time — but **only on QA, with no execution gate** ([Router-R1,
  arXiv:2506.09033](https://arxiv.org/abs/2506.09033); [RouteLLM,
  arXiv:2406.18665](https://arxiv.org/abs/2406.18665)). AXIS-A is conceded to
  them.
- **Learned budget-aware/structure-guided orchestration is NOT claimed.**
  MACA, Puppeteer, and AgentConductor already establish learned, task/budget-
  conditioned orchestration — but **on QA/math/function-or-competition code over a
  fixed pool, with no execution-authoritative patch gate** ([MACA,
  arXiv:2605.25746](https://arxiv.org/abs/2605.25746); [Puppeteer,
  arXiv:2505.19591](https://arxiv.org/abs/2505.19591); [AgentConductor,
  arXiv:2602.17100](https://arxiv.org/abs/2602.17100)). This axis is conceded to
  them, and APEX's headline is moved off "learned/structure-guided budget-aware
  orchestration" entirely.

APEX therefore claims only the unoccupied product. The
[critical review's own narrowed statement](APEX_NEXTGEN_CRITICAL_REVIEW.md)
("repo-level SWE search-policy generalization to a held-out vendor/model pool,
with no retraining, under execution-authoritative patch acceptance and
cost-matched budgets") is adopted as the headline, replacing any broader framing.
This sharpens — does not contradict — §19.2: the worker-representation mechanism
(the learned `CapabilityProfile` vector, §19.2.1) remains the ablatable novelty,
and the §19.2.2 conjunction is re-expressed as the AXIS-A × AXIS-B product so the
two now-conceded axes are explicitly attributed to prior art.

#### 24.1.3 Fallback paper positions (required-change 10)

The plan pre-commits to three nested fallback positions, ordered by the strength
of the result that actually lands. This is an honesty guard, not hedging: the
held-out-vendor split (§24.4) is the experiment that decides which position we
write up, and the decision rule is pre-registered before any held-out run.

| Position | Trigger condition | Paper shape |
|---|---|---|
| **(i) Main research claim** | Held-out-vendor generalization works: at matched token/$ budget the controller retains cost-quality-Pareto dominance on the held-out split (passes H1, §19.3 — neither falsification condition fires) | NeurIPS-grade research paper on the AXIS-A × AXIS-B product (§24.1.2) |
| **(ii) Systems / tooling paper** | Held-out generalization **fails** (H1 falsified) but durable vendor-neutral orchestration works: cross-vendor CLI/API workers behind normalized telemetry, durable journaled resume, execution-authoritative acceptance, and cost-matched Pareto reporting are demonstrated end-to-end | Systems/tooling paper — durable, vendor-neutral, execution-authoritative orchestration engine; held-out result reported as an honest negative (still publishable per §19.6) |
| **(iii) Engineering report** | Only v1 stabilization works: the §24.5 stabilization gates pass and the engine is reliable, but neither the generalization nor the full durable-orchestration story is demonstrated | Engineering report — **not** a splash paper; documents the stabilized substrate and the negative/under-powered findings |

The three positions are evaluated strictly in order (i → ii → iii); we report the
strongest one whose trigger holds, and we never present a position-(iii) result as
if it were position (i). This mirrors the §19.3.2 "honest priors" stance
(generalization is the most likely *clean* win; the single-best-model
quality-at-matched-cost win is the genuinely risky part) and gives reviewers an
explicit, falsifiable map from outcome to claim.

#### 24.1.4 Consistency with the Cardinal Safety Contract and accepted mechanisms

This addendum is consistent with the Cardinal Safety Contract (§13.1): execution
evidence remains authoritative; a soft signal may re-rank within an
execution-verified tier or downgrade an accepted candidate, but may never promote
an unverified candidate or suppress a candidate before execution evidence exists.
Nothing in §24 weakens that contract — the narrowed novelty *rides on* it (every
accept in §24.1.2 stays "gated exclusively by the execution-grounded acceptance
bar"), and §24.5's two-part invariant restatement (acceptance gate + ranking
scope) makes the contract's enforcement *more* precise, not less. The
accepted-mechanism inventory of §18 (Fusion Ledger) is unchanged: no mechanism is
added, removed, or re-dispositioned by this section. DeLM — the one cited system
the verification pass could **not** confirm (no arXiv ID, five negative
searches) — is explicitly dropped and is **not** cited as prior art or used as a
baseline anywhere in §24. Every in-place edit named in the response map (§24.1.1)
only removes a flagged overstatement (the "hardened substrate" confidence, the
"ultracode"-as-paradigm branding, and the §13.1 tuple sentence that claimed soft
keys sit below execution keys when `combined_score` is in fact the leading
sort key); none introduces a new claim. The result is a strictly tighter plan
that survives the review's prior-art and stability objections while preserving
the §19 conjunction spine and the execution-authoritative safety boundary intact.

### 24.2 Component-Collision Map and Two-Axis Novelty Thesis (branching, routing, search, verification are integrated prior art)

Critical-review finding #2 requires one concrete change: **stop presenting adaptive
branching, model routing, workflow search, and execution verification as
inventions.** This subsection executes that change as a buildable artifact, not a
disclaimer paragraph. It (a) maps every component of APEX-Ω's surface onto its
exact prior art with a falsifiable `owns` / `does NOT cover` column, (b) states
the two-axis novelty thesis and proves — per paper — that the *product* cell is
unoccupied, (c) records MACA as REAL prior art with the auditable
verification-discrepancy note, (d) sharpens AOrchestra to the definitive form, and
(e) fixes exact-ID discipline so namesake traps cannot leak into the bibliography.
It supersedes the looser language of §19.1 (the §19.1 table remains, but every row
that touches branching/routing/search/verification now defers to the falsifiable
columns here) and is consistent with the narrowed thesis of §19.2 and the
Cardinal-Contract framing of §7.4.

The canonical claim this subsection must keep everything consistent with:

> APEX-Ω's single defensible contribution is the **instantiation of no-retrain,
> held-out-vendor search/route-policy generalization under execution-authoritative,
> repo-level SWE patch acceptance with cost-matched budgets.** It is explicitly
> NOT a claim that no-retrain held-out routing is itself novel (Router-R1/RouteLLM
> own that for QA), and NOT a claim that learned budget-aware / structure-guided
> orchestration is novel (MACA/Puppeteer/AgentConductor own that over a fixed
> pool). The invention is the unoccupied **product** of the two axes.

#### 24.2.1 The five collision families (exact IDs; `owns` and `does NOT cover` are falsifiable)

Every cited paper below was confirmed real by direct WebFetch of its canonical
arXiv `/abs/` and (where it exists) `/html/` URL. The `does NOT cover` column is
written as a *falsifiable absence* — a property a reviewer can check against the
paper's own evaluation, not a rhetorical hedge. Any reviewer who finds one of
these `does NOT cover` cells to be false has falsified APEX-Ω's novelty claim,
which is the point.

**Family 1 — Learned orchestration (controller prunes/sequences agents for cost-quality).**

| System | Exact ID | What it OWNS | What it does NOT cover (falsifiable) |
|---|---|---|---|
| Puppeteer | [2505.19591](https://arxiv.org/abs/2505.19591) | RL controller that prunes/sequences a fixed agent pool for cost-quality (NeurIPS'25) | Fixed in-house pool; QA/GSM-Hard/MMLU-Pro/SRDD; **not SWE-bench**, **not execution-authoritative patch acceptance**, **not held-out-vendor** |
| MACA | [2605.25746](https://arxiv.org/abs/2605.25746) ([html](https://arxiv.org/html/2605.25746v1)) | Task/budget-conditioned structural prior (GraphSpec) + GRPO orchestration policy; +8.42% acc / 43.19% fewer tokens | **Fixed pool**; QA/math/function-code (HumanEval/MBPP/MMLU-Pro/ARC-C/SVAMP/GSM-Hard); **not repo-level SWE**, **not execution-authoritative patch acceptance**, **not open-pool cross-vendor**, **not held-out-vendor** |
| AgentConductor | [2602.17100](https://arxiv.org/abs/2602.17100) | RL-optimized DAG-topology orchestration for code generation | Function/competition code; fixed pool; **not repo-level SWE**, **not held-out-vendor**. (Distinct from "The Conductor" [2512.04388] — see §24.2.5.) |

**Family 2 — Workflow / inference-time search.**

| System | Exact ID | What it OWNS | What it does NOT cover (falsifiable) |
|---|---|---|---|
| AFlow | [2410.10762](https://arxiv.org/abs/2410.10762) | MCTS over code-represented workflows with reusable operators + execution feedback (ICLR'25) | Workflow-graph search, not patch repair; function-level benchmarks; **not held-out-vendor routing**, **not repo-level SWE acceptance** |
| AB-MCTS | [2503.04412](https://arxiv.org/abs/2503.04412) | Adaptive width-vs-depth branching (Thompson); Multi-LLM cooperative variant; beats fixed MCTS **only above ~64 calls** | General inference-time search algorithm; **not a learned cross-vendor route policy**, **not execution-authoritative patch acceptance as the search invariant** |

**Family 3 — Heterogeneous model routing.**

| System | Exact ID | What it OWNS | What it does NOT cover (falsifiable) |
|---|---|---|---|
| Router-R1 | [2506.09033](https://arxiv.org/abs/2506.09033) | **No-retrain descriptor-conditioned routing to unseen models** (e.g. Palmyra-Creative-122B) via price/latency/example-perf descriptors | **QA only** (7 general/multi-hop QA benchmarks); rule-based reward; **no repo-level SWE**, **no code execution / patch gate** |
| RouteLLM | [2406.18665](https://arxiv.org/abs/2406.18665) | **No-retrain transfer** when strong/weak models are swapped at test time | Strong/weak binary route on preference data; **no repo-level SWE**, **no execution-authoritative acceptance** |
| DAAO | [2509.11079](https://arxiv.org/html/2509.11079v1) | Difficulty-aware query-specific routing + capability/cost profiles | **Fixed LLM pool** (trained to pick ground-truth LLMs); GSM8K/MATH/HumanEval/MBPP/GAIA/MMLU; **no held-out-model**, **no repo-level SWE**, **no patch gate** |
| AOrchestra | [2602.03786](https://arxiv.org/abs/2602.03786) ([html](https://arxiv.org/html/2602.03786v1)) | Cross-vendor dynamic sub-agents `<Instruction,Context,Tools,Model>`; mixed Gemini/DeepSeek/Claude/GPT/Qwen pool; cost-aware routing | **Fixed dev pool** — see §24.2.4 for the sharpened, definitive characterization |

**Family 4 — Dynamic SWE subagents.**

| System | Exact ID | What it OWNS | What it does NOT cover (falsifiable) |
|---|---|---|---|
| AOrchestra | [2602.03786](https://arxiv.org/abs/2602.03786) | Runtime sub-agent creation over a vendor-mixed pool; SWE-Bench + Terminal-Bench; execution-authoritative SWE-Bench acceptance (official FAIL_TO_PASS/PASS_TO_PASS) | No held-out/unseen-model experiment; no no-retrain cross-vendor generalization (§24.2.4) |
| BOAD | [2512.23631](https://arxiv.org/abs/2512.23631) | Orchestrator + specialized sub-agents (localize/edit/**validate**) discovered via bandit; SWE-agent scaffold; SWE-bench-Verified/Live; **execution-gated** | Fixed agent design space; **not held-out-vendor routing** |
| OpenHands | [2407.16741](https://arxiv.org/abs/2407.16741) | Sandboxed Docker runtime; `AgentDelegateAction`; event-stream generalist agents (ICLR'25) | Single-scaffold platform; **not a learned cross-vendor route policy**, **not held-out-vendor generalization** |

**Family 5 — Execution-gated SWE repair (established execution-authoritative patch acceptance).**

| System | Exact ID | What it OWNS | What it does NOT cover (falsifiable) |
|---|---|---|---|
| AgentForge | [2604.13120](https://arxiv.org/abs/2604.13120) | Execution-grounded multi-agent SWE; mandatory Docker sandbox — every change must survive sandboxed execution before propagation | Single-vendor; fixed pool; **not held-out-vendor**. (NOT the RL low-code namesake [2410.19528] — §24.2.5.) |
| R2E-Gym | [2504.07164](https://arxiv.org/abs/2504.07164) | 8.1K executable SWE envs + hybrid execution/execution-free verifiers; 51% SWE-bench Verified (COLM'25) | Verifier/best-of-N selection, not cross-vendor route policy; **not held-out-vendor** |
| OpenHands | [2407.16741](https://arxiv.org/abs/2407.16741) | Sandboxed action execution validates patches | (as Family 4) |
| SWE-agent | [2405.15793](https://arxiv.org/abs/2405.15793) | Agent-computer interface that edits + **executes tests** on SWE-bench (foundational) | Single-model scaffold; **not learned cross-vendor routing** |
| Agentless | [2407.01489](https://arxiv.org/abs/2407.01489) | localize→repair→**validate** (test-execution filtering + re-rank), no autonomous tool-use | Fixed pipeline; single model; **not held-out-vendor** |
| HyperAgent | [2409.16299](https://arxiv.org/abs/2409.16299) | Planner/Navigator/Editor/**Executor** multi-agent; Executor verifies by execution | Fixed roles; **not held-out-vendor routing** |
| PatchPilot | [2502.02747](https://arxiv.org/abs/2502.02747) | Rule-based reproduce/localize/generate/**validate**/refine; <$1/instance | Fixed workflow; single model; **not held-out-vendor** |

**Disposition that follows from Family 5:** execution-gated, test-validated patch
acceptance is *established prior art*. APEX-Ω therefore **does NOT claim
execution-gated patch selection as novel** — it adopts it verbatim as the Cardinal
Safety Contract (§7.4, §13). BOAD additionally belongs to this family (its
validation sub-agents are execution-gated on SWE-bench), so it is double-listed.

#### 24.2.2 The two-axis novelty thesis

Decompose the entire prior-art surface into two orthogonal axes. Each axis is
already *owned* by published work; the cell at their product is not.

```text
AXIS-A  (no-retrain, held-out / unseen-vendor route-policy generalization)
        OWNED for QA by  Router-R1 (2506.09033), RouteLLM (2406.18665)
        — descriptor-conditioned, transfers to unseen models with NO retraining
        — but: QA only, NO execution gate

AXIS-B  (repo-level SWE  +  execution-authoritative patch acceptance  +  vendor-mixed pool)
        OWNED over a FIXED pool by  AOrchestra (2602.03786)
        — cross-vendor sub-agents, SWE-Bench official FAIL_TO_PASS/PASS_TO_PASS gate
        — but: fixed dev pool, NO held-out-vendor / no-retrain experiment

APEX-Ω  =  AXIS-A  ×  AXIS-B           # the product cell
        =  no-retrain held-out-vendor routing  UNDER  execution-authoritative repo-SWE acceptance
        =  a cell NO single paper occupies
```

The contribution is the *product*, evaluated under a pre-registered held-out-vendor
protocol (§19.3, §20) with separate training / calibration / inference cost
accounting (§24.2.3 ledger row "training/data cost"). The mechanism that makes the
product reachable is the learned capability/cost **profile vector** (never one-hot
vendor identity) of §19.2 — the same indirection that lets Router-R1 route to an
unseen model is lifted onto Axis B, where no prior system has applied it.

#### 24.2.3 Per-paper shortfall ledger (why the product cell is empty)

For each closest paper, the *exact* axis it fails. Every "NO" is the falsifiable
claim a reviewer checks against that paper's own evaluation. "Train cost"
distinguishes offline filtered-trajectory RL from APEX-Ω's online posterior profile
estimator — avoiding offline filtered-trajectory training is itself a cost-side
contribution (§19.2.1, baseline B5 in §24.x).

| Paper (exact ID) | Axis A: no-retrain held-out routing | Axis B: repo-SWE + exec-authoritative + vendor-mixed | Product cell | Train/data cost |
|---|---|---|---|---|
| Router-R1 [2506.09033](https://arxiv.org/abs/2506.09033) | **YES** (descriptors → unseen models, no retrain) | NO (QA only; no execution gate) | NO | RL on QA reward |
| RouteLLM [2406.18665](https://arxiv.org/abs/2406.18665) | **YES** (strong/weak swap at test time) | NO (no repo-SWE; no exec gate) | NO | preference-data SFT |
| AOrchestra [2602.03786](https://arxiv.org/abs/2602.03786) | NO (fixed dev pool; plug-and-play swaps **frameworks**, not models — §24.2.4) | **YES** (SWE-Bench/Terminal-Bench; FAIL_TO_PASS/PASS_TO_PASS; mixed pool) | NO | SFT + ICL over fixed pool |
| MACA [2605.25746](https://arxiv.org/abs/2605.25746) | NO (fixed pool) | NO (QA/math/function-code; no exec-authoritative patch gate) | NO | **GRPO + filtered high-quality trajectories** (substantial GPU) |
| DAAO [2509.11079](https://arxiv.org/html/2509.11079v1) | NO (fixed LLM pool; trained to pick ground-truth LLMs) | NO (function-level code; no patch gate) | NO | likelihood training over fixed pool |
| AB-MCTS / AFlow [2503.04412](https://arxiv.org/abs/2503.04412) / [2410.10762](https://arxiv.org/abs/2410.10762) | NO (search, not a route policy) | NO (general / workflow search; not exec-authoritative repo-SWE acceptance) | NO | search-time, no learned route policy |

No row has YES in **both** axis columns. The product cell is therefore unoccupied
across the closest published work — the falsifiable core of APEX-Ω's novelty.

#### 24.2.4 AOrchestra — the closest collision, sharpened to the definitive form

The §19.2 hedge ("stops short of") is **upgraded** to the verified, definitive
statement (confirmed by direct WebFetch of [2602.03786](https://arxiv.org/abs/2602.03786)
and its [HTML body](https://arxiv.org/html/2602.03786v1); the terms "held-out",
"unseen model", "without retraining" do not appear):

> **AOrchestra does NOT evaluate held-out/unseen models or no-retrain cross-vendor
> generalization.** All of its routing / SFT / ICL operates over a **fixed, known
> development pool**, and its only "plug-and-play" test swaps sub-agent
> **FRAMEWORKS** (ReAct-style vs Mini-SWE-style), **not models**.

This *strengthens* APEX-Ω's defensibility rather than weakening it: AOrchestra
covers the entire Axis-B system surface (cross-vendor dynamic sub-agents,
SWE-Bench + Terminal-Bench, cost-aware orchestration, execution-authoritative
acceptance via the official harness) yet leaves Axis A — the held-out-vendor /
no-retrain generalization — completely unaddressed. AOrchestra is therefore the
**fixed-pool ceiling baseline** on the SWE/execution axis (§24.x baselines), the
exact ceiling APEX-Ω must exceed on the held-out split.

#### 24.2.5 Exact-ID discipline (dodge the namesake traps)

Two confirmed namesake collisions can corrupt the bibliography. Cite the exact ID;
a coding agent populating §23.6 must use these and reject the wrong sibling:

| Intended system | CORRECT ID | WRONG namesake — do NOT cite |
|---|---|---|
| AgentConductor (topology-evolution code-gen) | [2602.17100](https://arxiv.org/abs/2602.17100) | "The Conductor" [2512.04388](https://arxiv.org/abs/2512.04388) |
| AgentForge (execution-grounded SWE) | [2604.13120](https://arxiv.org/abs/2604.13120) | RL low-code framework [2410.19528](https://arxiv.org/abs/2410.19528) |

#### 24.2.6 MACA is REAL prior art — verification-discrepancy note and corrected protocol

MACA ([2605.25746](https://arxiv.org/abs/2605.25746), "Multi-Agent Coordination
Adaptation via Structure-Guided Orchestration"; Li, Chen, Sun, Wang; submitted
25 May 2026) is **confirmed real**. It contributes a task/budget-conditioned
structural prior (**GraphSpec**) plus a **GRPO** orchestration policy, reporting
+8.42% avg accuracy / 43.19% fewer tokens over adaptive MA baselines
(DyLAN/AgentVerse/MacNet/AgentPrune/MaAS/Puppeteer) on
HumanEval/MBPP/MMLU-Pro/ARC-C/SVAMP/GSM-Hard, with ablations of 14.45% accuracy
drop without the structural prior and 19.40% token-cost surge without the policy
(all confirmed by direct WebFetch of [/abs/](https://arxiv.org/abs/2605.25746)
and [/html/](https://arxiv.org/html/2605.25746v1)).

**What MACA does NOT cover (falsifiable):** fixed pool; QA/math/function-code; NOT
repo-level SWE; NOT execution-authoritative patch acceptance; NOT open-pool
cross-vendor; NOT held-out-vendor. Learned **structure-guided budget-aware
orchestration is conceded to MACA** and is never claimed as APEX-Ω's headline
(§19.2 novelty reframe). MACA enters APEX-Ω as: a §19.1 disclaimer row, a baseline
(B5: GraphSpec-like structure-guided orchestration ported to repo-SWE workers under
APEX's Normalized Executor + standardized scaffold + agent-unreachable grader, run
on both the cost-matched Pareto and the held-out-vendor split), an ablation (A12:
GraphSpec-like structural-prior + action-mask comparator swapped for the
profile-conditioned controller — hypothesized to match on the fixed-pool cost axis
but collapse to the fixed-pool ceiling on the held-out split), and a first-class
training/data-cost axis (its GPU-hours + filtered-trajectory count reported
separately from APEX's online estimator).

**Auditable verification-discrepancy note (protocol corrected).** An earlier
research pass falsely concluded MACA did not exist. Root cause: the arXiv
abstract-only page does **not** contain "GraphSpec", "GRPO", the benchmark names,
or the baseline names — those appear only in the body — and the WebSearch index had
not crawled this ID, so an **abstract-only / WebSearch-only check produced a false
negative.** Correction, now binding for all verification passes:

> Verify arXiv papers by **direct WebFetch of the canonical `/abs/` AND `/html/`
> URLs**. Never conclude existence/non-existence from **WebSearch alone**, and never
> from the abstract page alone (body-only terms like GraphSpec/GRPO will be missed).

**Explicit DeLM exclusion.** "DeLM" could **not** be verified: no arXiv ID, five
negative targeted searches (multiple spellings/expansions), and it was cited with
no link while every peer carried an arXiv URL. **DeLM is dropped — it is NOT cited
as prior art or as a baseline anywhere in APEX-Ω.** If a real system was intended,
it must be named and verified first (candidates only, not adopted: LDB
[2402.16906](https://arxiv.org/abs/2402.16906), Lingma SWE-GPT, DebugRepair
[2604.19305](https://arxiv.org/abs/2604.19305)).

### 24.3 Expanded Baseline and Ablation Matrix (B5-B8 + A12) under APEX's Repo-Level-SWE + Execution-Authoritative + Cost-Matched Contract

This subsection answers critical-review **finding #6** ("the evaluation plan is strong but incomplete — the missing baseline set is serious") and **required-change #1** (add MACA, AOrchestra, Router-R1/DAAO, RouteLLM, AFlow, AB-MCTS, AgentConductor to related work and baseline/ablation planning). It extends the §20.2 baseline table (B0-B4) and the §20.3 ablation table (A1-A11) with four new baseline families (**B5-B8**) and one new ablation (**A12**), and tightens the §20.6 minimum reviewer-demanded result so that dominance must hold over the *closest published collisions*, not only over the easy anchors. Every new comparator is specified as a *fair, non-strawman* port — it is given the same standardized scaffold, the same Normalized Executor, the same agent-unreachable grader, the same cost-matched budget, and is evaluated on **both** the cost-matched Pareto frontier **and** the held-out-vendor split defined in §24.4. The reframed novelty thesis (§24.1) and the collision map (§24.2) are the source of truth this subsection stays consistent with: APEX-Ω's only headline claim is the *product* of AXIS-A (no-retrain held-out routing, owned for QA by Router-R1/RouteLLM) and AXIS-B (repo-level SWE + execution-authoritative acceptance + vendor-mixed pool, owned over a fixed pool by AOrchestra). Each new baseline is positioned precisely to attack one axis or their conjunction.

One disposition is fixed up front: **DeLM is excluded from the baseline set.** Five targeted searches found no paper, repo, or leaderboard system by that name, and the review cites it with no arXiv ID while every peer citation carries one. We do not cite it as prior art and do not implement it as a baseline; if a real system was intended it must be re-identified with a verifiable ID before it can enter the matrix.

#### 24.3.1 The new baseline families (B5-B8)

All of B5-B8 run through the identical contract already mandated for B0-B4 in §20.7: one **Normalized Executor** (`run_worker(spec, task, scaffold)`, §3.2 / plan:4760-4773), the same SWE-Agent/mini-swe-agent standardized scaffold (so scaffold-vs-orchestration confound is removed; scaffold alone swings results 15-21 pts), and the **separate, agent-unreachable grader** process (hidden/read-only fail-to-pass + pass-to-pass behind the per-rollout `fcntl` lock and network-default-none sandbox; the upstream Docker harness pinned by digest is the only published verdict). No baseline may read or mutate the hidden suite, and a synthesized "pass" yields `errors=1`, never `passed=1` (v1 silent-`rc==0`→errors rule). Each baseline is a comparator we intend to *beat or match*, never our headline.

| ID | Baseline family | Faithful citation (verified) | What it isolates (which axis it attacks) | Why it is a fair, non-strawman port |
|---|---|---|---|---|
| **B5** | MACA / GraphSpec-like structure-guided orchestration | MACA, [arXiv:2605.25746](https://arxiv.org/abs/2605.25746) (Li, Chen, Sun, Wang; GraphSpec structural prior + GRPO policy) | AXIS-B incumbent on the *learned-orchestration* sub-axis: a learned, task/budget-conditioned structural prior + RL policy over a **FIXED** agent pool, ported to repo-level SWE workers. The closest collision to "learned budget-aware orchestration," which §24.1 concedes to MACA. | Given the full MACA recipe (structural prior + GRPO-style policy), trained on *our* pool to parity, not a degraded re-implementation. It is the strongest learned-orchestration comparator; APEX must beat it on the cost-matched Pareto and reveal its **fixed-pool ceiling** on the held-out split. |
| **B6** | AOrchestra-style dynamic sub-agent orchestration | AOrchestra, [arXiv:2602.03786](https://arxiv.org/abs/2602.03786) (Ruan et al.; sub-agents as `<Instruction, Context, Tools, Model>`; mixed Gemini/DeepSeek/Claude/GPT/Qwen pool; SWE-Bench + Terminal-Bench; execution-authoritative FAIL_TO_PASS/PASS_TO_PASS) | The fixed-pool ceiling on the SWE/execution axis: **vendor-mixed but FIXED dev pool, no learned held-out routing.** This is the system-surface twin of APEX and the single closest collision. | Run with its own dynamic-subagent creation and cost-aware orchestration intact over the *same* mixed pool. It is fair because it gets vendor diversity but, by construction, *cannot route an unseen vendor* (no held-out/no-retrain mechanism). The held-out split is exactly where B6 must reveal whether APEX's win is open-pool generalization or merely vendor diversity. |
| **B7** | Descriptor / transfer-routing baseline | Router-R1 [arXiv:2506.09033](https://arxiv.org/abs/2506.09033); DAAO [arXiv:2509.11079](https://arxiv.org/abs/2509.11079); RouteLLM [arXiv:2406.18665](https://arxiv.org/abs/2406.18665) | AXIS-A incumbent: **no-retrain transfer to unseen models** via descriptor-conditioned routing — the capability §24.1 concedes to Router-R1/RouteLLM. | Given the **SAME profile metadata as APEX** (no information asymmetry — directly answering review finding #7's "baselines may be handicapped if APEX gets richer metadata"), but **no execution gate beyond APEX's mandatory acceptance harness**. Fair because it owns the held-out axis; it must fail on the repo-level-SWE × execution conjunction APEX claims. |
| **B8** | Adaptive workflow-search baseline | AB-MCTS [arXiv:2503.04412](https://arxiv.org/abs/2503.04412); AgentConductor [arXiv:2602.17100](https://arxiv.org/abs/2602.17100) (topology-evolution code-gen; distinct from "The Conductor" 2512.04388); AFlow [arXiv:2410.10762](https://arxiv.org/abs/2410.10762) | The inference-time/workflow-search ceiling: adaptive-branching / topology-search controller with a **hand-designed or learned workflow-graph policy over a fixed pool** — no profile-based open-pool routing. | Given the same expansion budget as APEX's bounded adaptive-branching (§9) so the comparison isolates *open-pool routing* from *search depth*. Fair because it owns adaptive branching/topology search; it cannot transfer that search policy to an unseen vendor without retraining. |

B5 and B6 are the **closest collisions** and therefore the load-bearing additions: B5 contests the learned-orchestration claim, B6 contests the system surface. B7 and B8 are the axis-ceiling probes (descriptor routing and workflow search respectively). The four together close the gap the review flagged: the original B0-B4 set tested *single model / best-of-N / v1 / re-trained orchestrator (Puppeteer/AFlow) / static cross-vendor*, but never a vendor-mixed dynamic-subagent system (B6) or a descriptor-transfer router (B7) under the execution-authoritative contract — which is exactly where prior art has no published result.

#### 24.3.2 The new ablation (A12) — GraphSpec-like structural prior + action-mask comparator

A12 follows the §20.3 discipline (toggle exactly one mechanism; fail open to the heuristic floor when disabled) and is the structural-prior analogue of A9 (capability-profile vs one-hot). It swaps APEX's profile-conditioned controller for a MACA-style structural prior, holding everything else fixed.

| # | Ablation | Arms | Hypothesis / expected | Maps to mechanism |
|---|---|---|---|---|
| **A12** | GraphSpec-like structural-prior + action-mask vs APEX profile-conditioned controller | (a) APEX profile-conditioned controller / (b) GraphSpec-style structural prior + action-mask, rest of stack fixed | On the **fixed-pool cost axis** A12 should *match* APEX (MACA's learned structure-guided budget-aware orchestration is genuinely strong — this is the honest concession). On the **held-out-vendor split** A12 must **collapse to the fixed-pool ceiling**, because a structural prior over a fixed agent set has no representation for an unseen vendor. | Open-pool profile controller (§14/§19.2) vs structure-guided fixed-pool orchestration (B5/MACA) |

A12 is a *falsification signal complementary to A9*. A9 attacks the representation from below (one-hot can't generalize); A12 attacks it from a published-prior-art direction (a *learned structural prior* also can't generalize past its fixed pool). If A12 matches APEX on the held-out split, the profile representation is doing no work and the headline mechanism is unfounded — reported as a negative result, itself publishable (§19.6).

**Open decision (must be resolved before the eval freeze and recorded in the §18 Fusion Ledger).** B5/A12 may be instantiated either as **(i) a faithful GraphSpec re-train** — substantial GPU training of the GRPO-style policy on filtered high-quality trajectories over our pool — or **(ii) a lighter structural-prior + action-mask comparator** that reuses APEX's online profile estimator as the per-edge relevance signal and applies a GraphSpec-shaped coordination mask without a full RL re-train. The choice must be justified as fair, non-strawman either way:

- **(i) faithful re-train** is the strictly stronger comparator and the more defensible against a "you handicapped MACA" objection, but it costs real GPU-hours and a filtered-trajectory pipeline, and its result is only valid if the re-train converges to within parity of MACA's *reported* gains on its own benchmarks (a pre-registered sanity check: reproduce MACA's +8.42% accuracy / 43.19% token-reduction direction on HumanEval/MBPP before porting to repo-level SWE, else the port is not trustworthy).
- **(ii) lighter comparator** is fair *only if* its structural prior is given the same information APEX's controller has and its action-mask is no more restrictive than MACA's published GraphSpec masking; it is *not* fair if it is degraded below MACA's reported mechanism. The decision criterion: choose (ii) only if a pilot shows (ii) reaches the fixed-pool cost-axis parity that the hypothesis predicts for A12; if (ii) underperforms APEX on the *fixed* pool, that is evidence of a strawman and we must escalate to (i).

The default recommendation is **(ii) with a (i) escalation trigger**: start with the lighter comparator gated by the fixed-pool parity pilot, and escalate to the faithful re-train if (ii) cannot reach fixed-pool parity, because a comparator that loses on the fixed pool cannot honestly demonstrate the held-out collapse that is A12's whole point.

#### 24.3.3 Training / calibration / inference cost ledger (B5 / A12, vs APEX)

Per review required-change #1 and finding #1, **training cost and data cost are first-class reported axes** for the learned-orchestration comparators, because *avoiding offline filtered-trajectory training is itself an APEX cost-side contribution*. MACA's reported setup uses substantial GPU training plus filtered high-quality trajectories; APEX's controller uses an online posterior profile estimator with no offline RL re-train (§19.2.1: "estimating a worker's profile is cheap and online"). We report three cost columns *separately* — they must never be summed into a single "cost" number, and the cost-matched Pareto (§20.4) matches only the **inference** column across systems while reporting training/calibration as standalone disclosed cost.

| Cost axis | Definition (unit) | B5 / A12 (GraphSpec-like) | APEX-Ω | Why reported separately |
|---|---|---|---|---|
| **Training** | GPU-hours + hardware for the GRPO-style policy re-train | Substantial (full RL re-train if option (i); near-zero if option (ii) reuses the online estimator) | ~0 (no offline policy re-train; online posterior estimator only) | A re-train that beats APEX at inference but costs GPU-weeks is not "free" — disclosed, never folded into inference. |
| **Data / calibration** | Count + filtering cost of high-quality trajectories; calibration-task budget | Filtered high-quality trajectory corpus (MACA-style); count + filter-pass cost logged | Calibration only on the §24.4 allowed-calibration-task set; no filtered-trajectory corpus | Trajectory filtering is a hidden labor/compute cost; APEX's online estimation avoids it (the cost-side contribution). |
| **Inference** | Tokens + normalized cross-vendor $ per task at eval time | Per-task tokens/$ on the standardized scaffold | Per-task tokens/$ on the standardized scaffold | The **only** column the cost-matched Pareto matches across systems (`cost_match_tol=0.10`, §20.5). |

The ledger is emitted as a typed artifact so the comparison is auditable and machine-checkable, not prose:

```python
@dataclass(frozen=True)
class OrchestratorCostLedger:
    system_id: str                  # "B5" | "A12" | "apex_omega"
    train_gpu_hours: float          # 0.0 for APEX (no offline policy re-train)
    train_hardware: str             # e.g. "8xH100" | "none"
    data_trajectory_count: int      # filtered high-quality trajectories; 0 for APEX
    data_filter_pass_cost_usd: float
    calibration_task_count: int     # §24.4 allowed-calibration set only
    inference_tokens_per_task: float
    inference_usd_per_task_normalized: float   # cross-vendor normalized $ (§16)
    profile_estimator_online: bool  # True for APEX; False for offline-RL comparators
```

A write-time harness assertion refuses to emit any APEX-vs-B5/A12 delta claim that compares the *summed* cost columns; only the `inference_*` columns may enter a cost-matched point comparison, and `train_*` / `data_*` must be reported as standalone disclosed figures in the same table.

#### 24.3.4 Updated minimum reviewer-demanded result

This supersedes the §20.6 / §20.2 minimum result. The original bar — dominance over B0, B1, B3 on the cost-matched Pareto plus the held-out split — is *necessary but no longer sufficient*, because it omits the closest collisions. The updated bar:

1. **Cost-matched Pareto dominance** on a contamination-resistant benchmark (SWE-bench Pro public *and* commercial; corroborated on a SWE-bench-Live month and the private rotating set): the open-pool controller's resolved-rate-vs-$ frontier **strictly dominates**, with paired bootstrap CIs on the deltas:
   - **B0** (strongest single model),
   - **B1** (cost-equal verified best-of-N),
   - **B3** (re-trained published orchestrator, Puppeteer/AFlow, same pool),
   - **B5** (MACA-like structure-guided orchestration) — *the closest learned-orchestration collision*,
   - **B6** (AOrchestra-like fixed-pool dynamic sub-agent orchestration) — *the closest system-surface collision*.
2. **Held-out-vendor split, no retraining** (§24.4): at test time the pool contains a vendor/model absent at train time; the controller routes it via learned capability/cost profiles and still dominates **B4** (static cross-vendor routing), **B6** (fixed-pool dynamic subagents), and **B7** (descriptor routing with identical metadata). The B6 and B7 deltas are the decisive disambiguators:
   - dominance over **B6** on the held-out split shows the win is **open-pool generalization**, not vendor diversity alone (B6 has the diversity but not the held-out mechanism);
   - dominance over **B7** shows the win is the **execution-authoritative repo-level-SWE conjunction**, not descriptor routing alone (B7 has the held-out routing but not the execution gate).
   - **A12** must collapse to the fixed-pool ceiling on this split (matching APEX on the fixed pool, failing on held-out) — the mechanism falsification check.

If (1) holds but (2) fails, we have a strong systems result but **not** the novelty claim, and the paper is reframed exactly as §20.6 prescribes (search/economy as bounded amplifiers; execution evidence as steering signal and brake; best-of-N as the floor). We never overclaim open-pool generalization off a same-pool result, and — per the reframed thesis (§24.1) — we never claim learned/structure-guided budget-aware orchestration (conceded to MACA) or no-retrain descriptor routing (conceded to Router-R1/RouteLLM) as the headline; only their unoccupied product is the contribution.

#### 24.3.5 Buildable CI gates for the expanded matrix

These gates extend the §20 harness so a coding agent (Codex or Claude Code worker) can build and enforce the matrix without ambiguity. Each is a named acceptance test that must pass for a run to reach "publishable."

| Gate | Asserts | Fails the run when |
|---|---|---|
| `gate_baseline_contract_uniform` | B5-B8 each ran through the *same* Normalized Executor, scaffold hash, grader-process id, and budget as B0-B4 | any baseline used a different scaffold hash, reached the hidden suite, or ran an unmatched inference budget |
| `gate_b5_a12_disposition_recorded` | The faithful-re-train-vs-lighter-comparator decision (24.3.2) is recorded in the Fusion Ledger with its justification and the fixed-pool parity pilot result | the disposition is unset, or option (ii) was used without passing the fixed-pool parity pilot |
| `gate_b5_maca_sanity` | If option (i): the re-train reproduces MACA's reported direction (+accuracy / −tokens) on its own benchmarks before the SWE port | the re-train does not converge to MACA's reported parity on HumanEval/MBPP |
| `gate_cost_ledger_separated` | `OrchestratorCostLedger` emitted for B5/A12/APEX with `train_*`, `data_*`, `inference_*` populated and never summed | any delta claim compares summed cost, or `train_gpu_hours`/`data_trajectory_count` are missing for a learned comparator |
| `gate_b7_metadata_parity` | B7 (descriptor routing) received byte-identical profile metadata to APEX (no information asymmetry, finding #7) | B7's metadata hash differs from APEX's per-node metadata hash |
| `gate_min_result_collisions` | The §24.3.4 dominance set (B0, B1, B3, B5, B6 on Pareto; B4, B6, B7 on held-out) is computed with paired bootstrap CIs | any required dominance comparison is missing or claimed without a paired CI |
| `gate_delm_excluded` | DeLM appears nowhere as a cited baseline or related-work entry | DeLM is referenced as prior art or a comparator |

These gates make the expanded matrix a buildable, auditable contract rather than an aspiration: the disposition decision is forced and recorded, the cost ledger is structurally separated, the closest collisions are mandatory dominance targets, and the unverifiable system (DeLM) is statically barred from the result.

### 24.4 Pre-Registered Held-Out-Vendor Protocol, Capability-Profile Construction Rules, Per-Decision Logging, and OOD Diagnostics

This subsection answers review findings #6 ("missing baseline set is serious") and #7 ("open-pool held-out-vendor generalization is underspecified"), and discharges required-change #7 ("pre-register the held-out-vendor evaluation protocol and capability-profile construction rules"). It does **not** redesign the controller: it freezes a contract over the existing §14.4 `BackendProfile`/`TaskFeatures`, the §14.9 `ControllerDecision` journal, the §13 Cardinal-Contract selector, and the §20 evaluation harness, then adds exactly the leakage controls, ledger separation, baselines, and diagnostics the review demands. The narrowed novelty claim it must support is the verified two-axis product (the [routing prior-art map](https://arxiv.org/abs/2602.03786)): no-retrain held-out-vendor routing (owned for QA by [Router-R1, arXiv:2506.09033](https://arxiv.org/abs/2506.09033) / [RouteLLM, arXiv:2406.18665](https://arxiv.org/abs/2406.18665)) **×** repo-level SWE + execution-authoritative acceptance + vendor-mixed pool (owned over a *fixed* pool by [AOrchestra, arXiv:2602.03786](https://arxiv.org/abs/2602.03786), which has **no** held-out/unseen-model experiment — its only plug-and-play test swaps sub-agent *frameworks*, not models). The cell at the product is unoccupied; this protocol is what makes APEX's occupation of it falsifiable.

Everything below is frozen-before-any-run. The freeze is a content-addressed artifact (`heldout_protocol.lock`, a hash over the split manifest + envelope spec + pass/fail predicate) committed to the repo before a single eval rollout executes; the harness refuses to score a run whose `protocol_hash` is absent from or mismatched against this lock.

#### 24.4.1 The frozen train/calibrate/held-out split

The pool is partitioned **once**, before any run, into named vendor sets and named task-id sets, and the partition is committed as `split_manifest.json`. No retraining and no online policy update on the held-out vendor's tasks is permitted after the freeze.

| Set | Vendors | Purpose | Mutable after freeze? |
|---|---|---|---|
| `train_vendors` | `{A, B}` (e.g. two of {codex_cli, claude_cli, ...}) | Stage-0 bandit / Stage-1 GEPA / Stage-2 RL fit (§14.5–14.6) | No |
| `calibration_corpus` | A+B only | measure every `cap_*` axis of §14.4 `BackendProfile`; fit the §14.7 calibrator | No |
| `eval_corpus` | A+B+**C** | the scored Pareto + held-out split | No |
| `heldout_vendor` | `{C}` (named, e.g. a third vendor/model) | added **at eval only**, with NO controller retraining and NO online update on C's tasks | No |

Buildable assertions (CI gate `g_split_frozen`, runs before any eval rollout):

```python
def assert_split_frozen(lock, manifest):
    assert manifest.heldout_vendor not in manifest.train_vendors        # C never trained
    assert manifest.heldout_vendor not in manifest.calibration_vendors  # C never calibrated
    assert set(manifest.calibration_task_ids) & set(manifest.eval_task_ids) == set()  # 24.4.4
    assert blake2b(canonical_json(manifest)) == lock.split_hash         # frozen-before-run
    assert lock.committed_at < first_eval_rollout_started_at            # freeze precedes runs
```

At eval, the harness loads C's profile produced *only* from C's calibration observations bootstrapped through the cold-start path (§24.4.4); the policy weights `theta` are byte-identical to the post-train checkpoint (asserted by `policy_version` equality across the whole C cohort in §14.9, see §24.4.5). Any gradient step keyed to a `decision_id` whose `vendor_model` contains C invalidates the run (`g_no_heldout_update`).

#### 24.4.2 No vendor one-hot — config-build assertion + negative-control arm

The §14.4 contract is already "identity is metadata only — NEVER a feature the policy keys on." This subsection makes that *checkable at config-build time*, not just asserted in prose.

```python
def assert_no_identity_features(feature_spec, profiles):
    # The policy input is concat(TaskFeatures, BackendProfile.cap_*/cost_*/reliability, pref).
    # backend_id / model_id / cli_version / alias strings MUST NOT enter the feature tensor.
    banned = {"backend_id", "model_id", "cli_version", "vendor", "alias",
              "provider", "model_family"}
    assert banned.isdisjoint(feature_spec.feature_names)            # static check
    # Runtime witness: permuting identity strings must not change the routed action.
    base = route(task, profiles)
    perm = route(task, [p._replace(backend_id=shuffle(p.backend_id),
                                   model_id=shuffle(p.model_id)) for p in profiles])
    assert base.action_distribution == perm.action_distribution    # identity-invariant
```

**Negative-control arm `A9-onehot` (extends §20.3 A9, the H1 falsification ablation).** A sibling controller is trained with a one-hot `(vendor, model)` arm-per-identity instead of the profile vector. Pre-declared expectation: `A9-onehot` **must fail to route C** — it has no arm for an identity unseen at train time, so it collapses to its fail-open heuristic on every C decision. If `A9-onehot` routes C as well as the profile controller, H1 is falsified and the open-pool claim is dead; this is a load-bearing honesty result, not a formality.

#### 24.4.3 Parity envelope (identical across all arms, including baselines)

Every arm — APEX, B0–B7 baselines, and every ablation — runs under one byte-identical envelope. The harness **refuses to emit a held-out delta** unless the envelope hashes match across the two arms being compared. This extends the §20.4 cost-match assertion ("refuses to emit a delta claim unless ...") to the full execution contract, so a "win" can never be an envelope artifact (review finding #7: "baselines may be handicapped if APEX gets richer metadata").

| Envelope field | Binding | Source of truth |
|---|---|---|
| `verifier_hash` | upstream Docker harness digest + Cardinal-Contract selector config (§13.9) | §13 / §20.7 grader, agent-unreachable |
| `budget_hash` | per-task token/$ cap + wall-clock policy (§14, §20.4) | `eval/stats.yaml` |
| `tool_perms_hash` | sandbox caps + tool allowlist + network policy (default-none) | §20.7 `WorkerSpec.sandbox_caps` |
| `scaffold_hash` | standardized SWE-Agent/mini-swe-agent shell (§20.2) | §20.7 `run_worker(scaffold)` |
| `scrubber_hash` | leakage-scrubber assertion set (§20.1) | §20.1 `scrub_sandbox` |

```python
def emit_heldout_delta(arm_a, arm_b):
    for f in ("verifier_hash","budget_hash","tool_perms_hash","scaffold_hash","scrubber_hash"):
        assert getattr(arm_a, f) == getattr(arm_b, f), f"envelope mismatch on {f}"  # byte-identical
    return paired_bootstrap_delta(arm_a, arm_b)        # only now is a delta admissible
```

The verifier is the same agent-unreachable upstream Docker harness for *all* arms (§13.7, §20.7): a `sys.exit(0)`/pytest-patch never reads as pass for APEX *or* for a baseline. C's tasks are graded by the identical hidden fail-to-pass/pass-to-pass suite used for A+B.

#### 24.4.4 Capability-profile construction rules (the leakage firewall)

Finding #7's central risk is that `cap_*` axes leak evaluation behavior. The rules below make profile construction auditable and content-addressed. They constrain only *how* the §14.4 `BackendProfile.cap_*` / `cost_*` fields are populated — no new fields.

1. **Disjoint-corpus rule.** Every `cap_*` axis is measured **only** on the `calibration_corpus`, with `set(calibration_task_ids) & set(eval_task_ids) == ∅` (asserted in `g_split_frozen`, §24.4.1). A profile axis touched by an eval task is a hard build failure.
2. **Measured-behavior-only rule.** `cap_*` is built **exclusively** from measured execution behavior — verifier outcomes (resolve/abstain/regression-survival), tokens, latency — never from `price tables / context-window sizes / alias strings / prompt templates / CLI failure signatures` (the exact identity-leak channels finding #7 enumerates). `cost_*` axes are the only metadata-sourced fields and are *not* identity-keyed (a measured $/ktoken is a number, not a name).
3. **Cold-start bootstrap + fail-open floor.** Below an observation floor `n_obs < cfg.profile.min_obs`, the axis bootstraps from the v1 capability *priors* and the controller fails open to the v1 failover ranker (§14.4 honesty note, §14.5 `route` floor). C necessarily starts under-floor, so C's first decisions are heuristic by construction — which is exactly the no-information-leak regime.
4. **Content-addressed `profile_hash`.** Each profile snapshot is hashed over its `(cap_* , cost_* , n_observations)` tuple; the hash is logged per decision (§24.4.5) so any reviewer can re-derive a profile from the calibration ledger and confirm it never saw an eval task.

```python
def build_capability_profile(vendor, calib_obs, cfg) -> BackendProfile:
    assert all(o.task_id in cfg.calibration_task_ids for o in calib_obs)     # rule 1
    assert all(o.source == "EXECUTION" for o in calib_obs)                   # rule 2 (measured)
    if len(calib_obs) < cfg.profile.min_obs:
        prof = bootstrap_from_priors(vendor)                                 # rule 3 (cold start)
    else:
        prof = ewma_profile(calib_obs)                                       # §14.4 EWMA
    prof = prof._replace(profile_hash=blake2b(canonical_json(prof.caps_costs())))  # rule 4
    return prof
```

#### 24.4.5 Per-decision routing log (extends §14.9 `ControllerDecision`)

The §14.9 journal already logs `task_features`, `profile_snapshot`, `policy_version`, `applied`, `library_enabled`, `vendor_model`, `effort`, and the verifier-authoritative outcome. This subsection **adds** the resolution and provenance fields required-change #7 names, so every C decision is fully auditable for drift and leakage. These are additive fields on the existing dataclass (no redesign).

| New field (on `ControllerDecision`) | Type | Why (finding #7) |
|---|---|---|
| `resolved_provider` | `str` | the concrete provider behind the chosen profile slot |
| `resolved_model_id` | `str` | canonical→launcher id; drift detector key |
| `cli_version` | `str` | provider CLI build; drift detector key |
| `context_window` | `int` | pinned; metadata, never a policy feature |
| `pricing_source` | `dict` | `{snapshot_id, retrieved_at}` — price table snapshot + timestamp |
| `tool_permissions` | `frozenset[str]` | parity-envelope witness (§24.4.3) |
| `prompt_hash` | `str` | blake2b of the rendered worker prompt |
| `profile_hash` | `str` | content-addressed profile (§24.4.4 rule 4) |

**Mid-cohort drift invalidation (CI gate `g_no_drift`).** A run is invalidated if `resolved_model_id` or `cli_version` changes within a single vendor cohort:

```python
def assert_no_midcohort_drift(decisions):
    for cohort in group_by(decisions, key=lambda d: d.resolved_provider):
        assert len({d.resolved_model_id for d in cohort}) == 1, "model drift mid-cohort"
        assert len({d.cli_version    for d in cohort}) == 1, "CLI drift mid-cohort"
```

This directly closes finding #7's "provider/model drift can invalidate runs unless exact versions ... are pinned."

#### 24.4.6 Required baselines under this contract

Findings #6/#7 and the routing verification both require two ceiling arms that *no published paper has run under a repo-level-SWE + held-out-vendor + execution-authoritative contract* — precisely where the prior art has no result. They join the existing §20.2 B0–B4 and the MACA-derived `B5` (the GraphSpec-like structure-guided baseline defined in the MACA-disposition subsection of this §24 addendum). All arms run under the §24.4.3 parity envelope and are scored on **both** the cost-matched Pareto split **and** the held-out-C split.

| ID | Baseline | Axis it pins | Pre-declared behavior on held-out C |
|---|---|---|---|
| B4 | static cross-vendor best-of-N + selector, no learned policy (§20.2) | "is the win just vendor-mixing?" | routes C by static rule; no learning to transfer |
| B5 | MACA/GraphSpec-like structure-guided orchestration ported to SWE workers (§24, MACA disposition) | learned structure-guided orchestration (conceded to [MACA, arXiv:2605.25746](https://arxiv.org/abs/2605.25746)) | **fixed-pool ceiling**: must retrain or degrade on C |
| **B6** | **AOrchestra-style fixed-pool dynamic-subagent orchestrator** ([2602.03786](https://arxiv.org/abs/2602.03786)) | the SWE+execution+vendor-mixed axis, **owned over a fixed pool** | **fixed-pool ceiling**: AOrchestra has no held-out experiment; it must **retrain or degrade on C** |
| **B7** | **Router-R1/RouteLLM-style descriptor-routing baseline** ([2506.09033](https://arxiv.org/abs/2506.09033) / [2406.18665](https://arxiv.org/abs/2406.18665)), same metadata as APEX | the no-retrain held-out-routing axis, **owned for QA** | **transfer ceiling**: descriptor-conditioned no-retrain routing, but with **no execution-authoritative patch gate** in its native form — here forced under APEX's execution gate |

B6 is the fixed-pool ceiling (it owns the SWE/execution surface but cannot generalize to C without retraining); B7 is the transfer ceiling (it owns no-retrain descriptor routing but only ever demonstrated it on QA with no execution gate). B7 is given the **same** descriptor metadata APEX uses (parity, §24.4.3) so any APEX win over B7 is attributable to the SWE + execution-authoritative instantiation, not richer metadata. This is the buildable form of the routing-verification recommendation to add "AOrchestra (fixed-pool ceiling) and a descriptor-routing baseline (transfer ceiling), both evaluated under APEX's repo-level SWE + held-out-vendor + execution-authoritative contract."

#### 24.4.7 OOD diagnostics

To keep the held-out result honest (finding #7: "hard to make clean"), the harness emits four OOD diagnostics for the C cohort, all derived from the §14.9 journal + verifier outcomes:

| Diagnostic | Definition | Read |
|---|---|---|
| Profile distance | distance of C's `profile_hash` vector to the convex hull of A+B `cap_*`/`cost_*` profiles | how far out-of-distribution C is; large distance ⇒ expect fail-open-heavy routing |
| Routing entropy (train vs eval) | Shannon entropy of `action`/`vendor_model` distribution, A+B-train cohort vs C-eval cohort | a collapse or spike signals the policy is not generalizing, it is degenerating |
| Per-`FailureClass` distribution shift | KL divergence of the 8-bucket `FailureClass` (§13.8) distribution, A+B vs C | isolates whether C fails differently (infra ≠ model miss) |
| Calibration-to-eval transfer plot | predicted route-value (Thompson `sampled_value`, §14.9) vs realized verified-quality, **on never-trained C** | the money plot: positive correlation on C is the direct evidence of no-retrain transfer |

The transfer plot is the diagnostic that operationalizes the splash claim: if predicted route-value does not correlate with realized verified-quality on C, the controller is not transferring, regardless of headline Pareto numbers.

#### 24.4.8 Separate cost ledger (training / calibration / inference)

Required-change #6/#7: report training, calibration, and inference cost as **three distinct line items**. The held-out claim is **inference-cost-matched** to the static-routing baseline (B4); training+calibration cost is *disclosed but excluded* from the per-task Pareto (it is a one-time fit cost, not a per-solve cost). This is also where APEX's cost-side contribution against [MACA, arXiv:2605.25746](https://arxiv.org/abs/2605.25746) is recorded — MACA's GRPO policy + filtered-high-quality-trajectory training is a substantial offline cost that APEX's online posterior profile estimator avoids.

| Ledger line | Counted | On the per-task Pareto? |
|---|---|---|
| `training_cost` | Stage-0/1/2 fit GPU-hours + rollouts (A+B only); for B5/B6: controller training; for B5: + filtered-trajectory data cost | **No** (disclosed, one-time) |
| `calibration_cost` | calibration-corpus rollouts to build profiles + fit the §14.7 calibrator (A+B only) | **No** (disclosed, one-time) |
| `inference_cost` | per-task token/$ at eval, including C | **Yes** — this is the cost-matched Pareto axis |

```python
def assert_inference_cost_matched(apex, b4, tol=cfg.cost_match_tol):  # §20.4 spine
    d = abs(apex.inference_cost - b4.inference_cost) / max(apex.inference_cost, b4.inference_cost)
    assert d <= tol, "held-out claim must be inference-cost-matched to static routing (B4)"
    # training_cost / calibration_cost are disclosed in the appendix, NOT on the Pareto.
```

#### 24.4.9 Pre-declared pass/fail for the splash claim

The pass/fail predicate is frozen in `heldout_protocol.lock` before any run, so it cannot be moved post hoc.

> **PASS:** On held-out vendor C, with no controller retraining and no online update on C's tasks, APEX's resolved-rate-vs-inference-$ frontier **strictly dominates** B4 (static cross-vendor routing) **and both ceiling baselines B6 (AOrchestra fixed-pool) and B7 (descriptor-routing transfer)**, with paired bootstrap CIs (§20.5) whose lower bound on every pairwise delta is `> 0`, under a byte-identical parity envelope (§24.4.3) and inference-cost match (§24.4.8). The §24.4.7 calibration-to-eval transfer plot must show positive predicted-vs-realized correlation on C.

> **FAIL → fallback:** If the cost-matched Pareto holds on the A+B splits but the C-split dominance or the transfer correlation fails, the contribution is **not** the open-pool splash; the paper falls back to the systems-paper framing of §24.1 (durable vendor-neutral execution-authoritative orchestration as the result), exactly as §20.6 already requires ("if (1) holds but (2) fails, we have a strong systems result but **not** the novelty claim"). We never overclaim open-pool generalization off a same-pool result.

```python
def heldout_splash_passes(C, lock):
    envelope_ok = all(emit_heldout_delta(apex_C, b).admissible for b in (B4, B6, B7))
    cost_ok     = assert_inference_cost_matched(apex_C, B4)
    dominates   = all(paired_bootstrap_delta(apex_C, b).ci_lower > 0 for b in (B4, B6, B7))
    transfer_ok = transfer_correlation(C) > lock.min_transfer_rho
    return envelope_ok and cost_ok and dominates and transfer_ok       # all four, or fall back to §24.1
```

This subsection is consistent with §14.4 (profiles, not one-hot), §14.9 (decision journal it extends), §13 (Cardinal-Contract selector as the shared verifier), §20.2–20.6 (baselines, cost-matching, the minimum reviewer-demanded result), and the §24 narrowed-novelty reframe: the only thing APEX claims here is the *instantiation* of no-retrain held-out-vendor routing under execution-authoritative repo-level SWE acceptance with cost-matched budgets — the verified-unoccupied product of the two axes, made falsifiable by a frozen protocol.

### 24.5 Staged Extraction (not Direct Extension) and the Minimal Vertical-Slice First Milestone

This subsection answers review findings #3, #9, #10 and required-change #4. The review's core charge is correct against the v1 source: APEX-Ω is described in places (Sections 2, 15, 22) as if v1 were a clean stable kernel onto which four new boundaries can be *added*, when in fact those boundaries are **verifiably absent** and the L2 behaviors they would normalize are **real but entangled** inside `orchestration/solver.py`'s serial stages and `rollout/engine.py`'s scheduler. We therefore replace "preserve the v1 kernel and extend it" with **staged extraction**: an ordered, independently-revertible sequence in which the old code path remains the fallback until parity is proven at each step, so a failed extraction step can never regress the headline number. The plan's existing roadmap (Section 22, Phase 0) is not wrong in *what* it builds; it is too coarse in *order*, and §24.5 makes the order buildable.

#### 24.5.1 What is absent (must be BUILT), what is real-but-entangled (must be EXTRACTED)

The four named kernel boundaries do not exist in v1 (verified against `/Users/sameertkhanna/Documents/apex/apex/apex`): there is no `class Executor` with `spawn -> run(ScopedTask) -> observe_diff()` (only tool-call executors `ACIToolExecutor` at `tools/aci.py:820` and `_SelectorToolExecutor` at `agents/selector_agent.py:112`); no `CapabilityProfile`/`negotiate()` (zero grep hits; the `capabilities/` package is episodic-memory facade code); no `(agent, stage, item)` workflow-graph journal that resumes incomplete nodes; and no `pipeline()` primitive (`_run_pipeline` at `solver.py:1488` and `_run_api_pass_then_invert_pipeline` at `rollout/engine.py:26137` are private helpers, not a combinator). These are **net-new builds**, not consolidations. What *is* real is the behavior those boundaries would wrap: the serial cascade `reproduce -> localize -> patch -> test` lives as local-variable stages inside `solver.py`, and the fan-out scheduler — worktree pools, preemption, stall expiry, emergency caps, quarantine, result promotion — lives inside `execute_rollout_requests` (`rollout/engine.py`). The extraction job is to lift entangled-but-correct behavior behind clean boundaries **without changing it first**.

| Boundary | v1 status (verified file:line) | §24.5 disposition |
|---|---|---|
| `Executor.spawn/run(ScopedTask)/observe_diff` | ABSENT; behavior entangled in `cli_backend.py run_structured_prompt` + `solver.py` stages | BUILD adapter that *wraps* `run_structured_prompt`, zero behavior change (Step A) |
| `CapabilityProfile`/`negotiate()` | ABSENT (no grep hits) | BUILD as read-only descriptor layer over `core/config.py:57-62` backends + `llm_routing.py` |
| `(agent, stage, item)` graph journal | ABSENT; durability EXISTS but is *not* a node journal | EXTRACT by upgrading the escrow WAL + transcript rehydration (Step B), not adding from scratch |
| `pipeline()` | ABSENT; v1 has barrier waves + serial in-rollout stages | BUILD behind a scaffolded/local-mode flag only (Step D), after stage extraction (Step C) |

#### 24.5.2 Durability is an UPGRADE, not a from-scratch build

The review's precision point (finding #3) is load-bearing and is corrected here: **durable execution already exists in v1**. v1 has durable JSONL transcript rehydration on restart (`orchestrator_in_container_agent.py:1138`) and a candidate-promotion escrow WAL (CCEDF: fsync-durable, flock-guarded, monotonic-seq, idempotent) at `rollout/engine.py` + `persistence/escrow.py`. What is *missing* is the **node-level `(agent, stage, item)` graph journal** that resumes incomplete workflow nodes. Step B is therefore framed precisely as **"upgrade the existing escrow WAL + durable transcript rehydration into a node-level graph journal,"** reusing the exact CCEDF ordering/idempotency discipline per node — never "add durability from scratch." Section 15.3.1 already specifies the per-call `JournalEntry`; §24.5 adds only the staging that gets us there safely, and adopts the workflow-journal schema of required-change #5 (request id, node id, stage, input hash, output artifact paths, model/backend, resolved model id, CLI version, prompt hash, profile hash, tool permissions, diff pointer, verifier status, terminal state, cost/tokens/latency, parent dependency ids).

#### 24.5.3 The ordered staged-extraction sequence (each step independently revertible)

Each step ships behind a flag whose default preserves the v1 path; the old path is the fallback until a **parity gate** proves the new path produces the same accepted artifact. A failed step is reverted by flipping one flag.

| Step | Action | Old path remains fallback? | Gate before advancing |
|---|---|---|---|
| A | **Wrap**: adapters only — `ScopedTask`/`ExecutorResult`/`AgentResult` over `run_structured_prompt`; **zero behavior change** | yes (adapter delegates) | byte-identical results vs unwrapped on smoke slice |
| B | **Journal around existing behavior**: upgrade escrow WAL + transcript rehydration into an append-only `(agent, stage, item)` node journal; record-only, does not yet drive control flow | yes (journal is observational) | journal replays existing run; no scheduling change |
| C | **Extract serial stage functions** preserving serial order | yes (serial executor flag) | **parity gate**: same accepted artifact + same verifier verdict as in-`solver.py` serial run |
| D | **`pipeline()` behind a scaffolded/local-mode flag ONLY** | yes (serial is default) | `pipeline` matches serial on scaffolded tasks; never touches live CLI yet |
| E | **kill/resume + cleanup tests BEFORE provider-backed** | yes | kill-mid-run -> resume -> same accepted artifact; zero leaked worktree/poisoned artifact |
| F | **Exact/artifact replay split** (exact replay kept UNCHANGED) | yes (exact replay untouched) | artifact replay rehydrates diffs + reruns verification; exact replay regression-clean |
| G | **Learned routing shadow-then-live** | yes (heuristic routing default) | shadow lift logged vs journaled decisions; live only behind fail-open flag |

The discipline mirrors finding #9's explicit instruction ("do not start by building pipeline over live CLI mode, V5, frontier search, or cross-branch sharing; first extract stage functions preserving serial behavior; then implement pipeline behind a feature flag for scaffolded/local mode; add kill/resume and cleanup tests before enabling provider-backed rollouts") and finding #10's two-product rule (keep exact replay, *add* artifact replay as a second mode — do not replace one with the other, which would lose debugging coverage).

#### 24.5.4 The seven explicit `pipeline()` contracts (finding #9)

`pipeline()` is the largest engineering risk because it must replicate, with no inter-stage barrier, every invariant that `execute_rollout_requests` enforces implicitly. These seven contracts are normative and each gets a property test (§24.5.6):

1. **Stage identity / idempotence.** Every `(item, stage)` unit has a `journal_key` (content hash of prompt + scoped inputs + base snapshot SHA + `item_id` + `stage`, per Section 15.3.1) and an `idempotency_key = run_id::node_id::attempt`. Re-running after a crash never double-applies an external side effect.
2. **Single-worktree-per-item ownership / handoff.** Exactly one worktree is owned by an item across all its stages; ownership is handed stage-to-stage under the per-rollout `fcntl` lock (Section 15.1.1), never shared concurrently.
3. **Cancellation that NEVER discards a succeeding artifact.** A cancel may stop *future* stages of an item; it may never discard a stage output that already verified — the escrow-WAL/CCEDF invariant (a cap/cancel can only stop work that is *not* winning).
4. **`WorktreePool`-capacity backpressure with reverse-drain.** When the pool is saturated, the scheduler drains later stages first (the reverse-range drain of Section 22.2.2 pseudocode) so an item never head-of-line-blocks behind siblings stuck at an earlier stage.
5. **Per-`(item, stage)` budgets that never abort a succeeding chain.** Budgets govern *whether to start more work*, not whether to stop a chain that is winning (§2.6 invariant 3); exhaustion may shape allocation but never kills an in-flight succeeding rollout.
6. **Terminal-stage-only promotion (Cardinal Contract preserved).** Only the terminal stage's output is eligible for promotion through the execution-authoritative selector (Section 13); intermediate stage outputs are artifacts, never accepted patches. Soft/learned signals re-rank within the execution-verified tier or downgrade only — finding #8's invariant carried verbatim.
7. **Cleanup / quarantine that leaks no worktree and no poisoned artifact.** Every terminal or failed item releases its lock and worktree on *every* exit path; the statsmodels failure mode (missing-workspace `FileNotFoundError`, 0.0% collapse, no orchestrator stage — `.apex_sm_validate_*` log) is the explicit anti-target. A poisoned artifact is quarantined, never promoted.

An eighth cross-cutting rule binds them all: **deterministic scheduling order is a pure function of `(item_id, stage_index, journal_seq)` — never wall-clock** (Section 15.2). This is what makes a kill-mid-run resume reproduce the same dispatch set.

#### 24.5.5 The minimal vertical slice is the literal first milestone (review items 1-5)

Per the review's Go/No-Go (lines 313-321), the first milestone is **not** the full APEX-Ω engine. It is exactly these five deliverables, each with a concrete acceptance:

| # | Deliverable | Maps to step | Acceptance |
|---|---|---|---|
| 1 | Existing serial rollout wrapped in `ExecutorResult`/`AgentResult` | A | wrapped run byte-identical to unwrapped |
| 2 | Append-only workflow journal for current rollout requests | B | journal schema (rc#5) populated; replayable |
| 3 | Kill-mid-run -> resume -> same accepted artifact smoke | E (slice subset) | resumed run reproduces accepted artifact + verdict |
| 4 | Selector invariant property tests (learned/LLM cannot promote execution-invalid patches) | F-adjacent | tests in §24.5.6 green |
| 5 | One scaffolded workflow using `agent()` + `parallel()` with exact replay UNCHANGED | A + exact-replay | exact replay regression-clean; no `pipeline()` yet |

Only **after** this slice is reliable do `pipeline()` (Steps C-D), learned routing (Step G), CTDG (built first as telemetry + test prioritization with a full-suite backstop, never an acceptance gate — required-change #9), and frontier search move into the engine. This makes the slice the gate that converts Section 22's Phase 0 from "build everything" into "prove the substrate, then ride it."

#### 24.5.6 Property/CI tests for the slice

```
test_selector_invariant_no_promote:        # finding #8
  inject arbitrarily high combined_score/critic/public on an UNACCEPTED cluster
  ASSERT it is NOT selected when any accepted cluster exists
test_reviewer_downgrade_only:
  ASSERT every reviewer hook can only flip verification.accepted True->False
test_ranking_scope:
  ASSERT _best_cluster_by_deterministic_ranking is only called on the output of
         _prefer_accepted_clusters / _build_selector_candidate_pool, never raw clusters
test_kill_midrun_identical_resume:         # finding #10 / Section 15.5
  SIGKILL after node seq==K; restart; ASSERT pre-kill agent() calls are journal REPLAYS,
  resumed trajectory identical in seq order, SELECTED WINNER byte-identical,
  AND its re-run verification reproduces the same accept decision and diff text
test_pipeline_parity_vs_serial:            # gate for Step C->D
  ASSERT pipeline() on scaffolded tasks yields the same accepted artifact as the serial executor
test_no_leaked_worktree_on_failure:        # contract 7, statsmodels anti-target
  force a stage failure; ASSERT lock released, worktree reclaimed, no orphaned workspace dir
```

#### 24.5.7 Routing distinction kept explicit (no conflation)

v1 routing is **static profile/stage-index lookup + config overrides + a failover ranker**: `get_llm_for_profile_stage` / `get_llm_for_stage` (`core/config.py:2529`/`:2560`) and `core/llm_routing.py`'s `_candidate_failover_rank`/`resolve_available_llm_config`. The one bandit in v1, `BanditRolloutAllocator` (`planning/manager.py:2152`), **allocates rollout COUNT, not model choice** — it must not be mistaken for learned model routing. Learned per-decision routing over normalized cost/capability profiles (Section 14.4, `BackendProfile` over attributes never one-hot identity) is therefore **net-new** and is staged **shadow-then-live** (Step G): in shadow mode the controller logs its would-be `(vendor, model, effort)` decisions against the journaled decisions of the heuristic that actually ran, so its lift is measured off-policy before it drives anything; live mode is gated behind a fail-open flag (`applied=False => value==baseline`). This keeps the canonical narrowed-novelty thesis intact: the contribution is no-retrain held-out-vendor route-policy generalization under execution-authoritative repo-level SWE acceptance with cost-matched budgets — not learned routing per se ([Router-R1](https://arxiv.org/abs/2506.09033), [RouteLLM](https://arxiv.org/abs/2406.18665) already own no-retrain descriptor routing for QA; [AOrchestra](https://arxiv.org/abs/2602.03786) owns the fixed-pool SWE surface; [MACA](https://arxiv.org/abs/2605.25746) owns learned structure-guided budget-aware orchestration over a fixed pool).

#### 24.5.8 One precision correction: schema-in-prompt is Codex-only (Gemini to-verify)

A small but auditable v1 claim correction (finding #3 caveat): the inline-schema fallback is bound to Codex CLI specifically — `planning/manager.py:3421` (and `:13287`) key `use_inline_schema` on `LLMBackend.CODEX_CLI`, embedding the schema in the prompt with `schema=None`. The Codex path is verified; a Gemini-specific inline-schema branch was **not located** in the inspected files, so the "for Gemini/Codex" parenthetical that appears in Sections 2-3 must be softened to **"inline-schema fallback for Codex CLI"** with the Gemini binding flagged to-verify. The mechanism is general (degrade-not-crash; embed schema + post-parse for any vendor lacking native structured output), but the *verified* binding today is Codex-only, and the plan must not overclaim a Gemini binding that source inspection did not confirm.

### 24.6 Pre-Implementation Stabilization Phase (Phase -1): Gates G1-G9 and Executable Benchmark-Fairness CI Gates F1-F5

This subsection answers critical-review **finding #4** ("Test And Benchmark History Contradict 'Hardened Substrate' Confidence") and **required-changes #3** (pre-implementation stabilization phase) and **#8** (executable benchmark-fairness checks). It inserts a **gated stabilization phase — Phase -1** — that must be green *before any Phase-0 extraction begins* (Phase 0 = §22.2, the engine/Executor/journal critical path). Phase -1 is not new feature work; it is a buildable, evidence-mapped checklist that converts the review's narrow, verified residual into nine code gates (G1-G9) plus five executable benchmark-fairness CI gates (F1-F5) wired to the real `apex/core/fairness_audit.py`. Every gate maps to a *specific verified failure*, so it is buildable, not aspirational, and each records its status + an evidence pointer in a `stabilization_gates.json` artifact.

#### 24.6.1 Posture reframe: active hardening, not a hardened substrate

The plan's earlier "hardened substrate" framing (e.g., §15 intro) overstated the current state. The honest posture, confirmed against the artifacts, is **"active hardening with focused regressions passing."** The single materially-overstated review sub-claim must also be corrected in the *other* direction: the review presented `.pytest_cache/v/cache/lastfailed` as a broad failure set spanning selector/target-runtime/rollout-deadline/config/Commit0/orchestrator. That cache is a **stale partial-run snapshot**. The verified current state is:

- The cache has 70 entries. **10 of 70 reference renamed/removed tests** (0 source matches today).
- Of the 60 still-collectable entries, **re-running yields 6 failed / 54 passed**. All 6 failures are in the `test_orchestrator.py` coverage-gap / strong-progress cluster.
- The 12 selector and 44 target-runtime entries (plus config, cli_backend, Commit0) **all pass now** — e.g. `test_selector.py::test_regression_pruning_runs_before_selection` and `test_target_runtime.py::test_target_runtime_fails_closed_for_dynamic_tools` both PASS on re-run.

So the true open regression set is **~6 orchestrator coverage-gap tests in `test_orchestrator.py` plus a stale cache** (most other collectable entries now pass; 10 reference removed tests) — **NOT** broad selector/target-runtime/config breadth. This does not rescue "hardened substrate"; it reinforces "active hardening with focused regressions passing," because most cache failures were fixed between the snapshot and now, leaving a focused residual. The same-day (2026-06-14) landing of 8+ GOLD_SUITE generality fixes is itself evidence of active hardening, not a settled substrate.

#### 24.6.2 The nine stabilization gates (G1-G9)

Each gate names its **verified failure**, a **buildable acceptance test**, and the **evidence pointer** recorded in `stabilization_gates.json`. Gates are CI jobs; all nine must be green on the pinned slice before Phase 0.

| Gate | Verified failure it closes | Buildable acceptance test (exit condition) |
|------|----------------------------|--------------------------------------------|
| **G1** Focused regression suite | 6 open `test_orchestrator.py` coverage-gap/strong-progress regressions; stale `lastfailed` | Green focused suite over selector / verifier / replay / worktree / provider-transport; **close the 6 orchestrator regressions**; regenerate `lastfailed` clean (0 collectable failures, 0 removed-test refs) |
| **G2** Trustworthy-local-validation | V4 #17/#11: local validation untrustworthy (103 tier_2_import_failed, 48 failed; Docker gate 104 harness_error vs 56 ship) | Local signal may not be used as acceptance/training evidence until it passes a trust gate: **Youden's J > 0** on the V4 slice (ties to §20.5, J = TPR − FPR strict > 0) with a tolerance band derived *empirically* on that slice, not assumed |
| **G3** Provider-transport conformance | V4 #5/#16: Codex hangs `apex_generation_duration_seconds = 7759.6s` (pytest-7373-68) / `8148.6s` (django-11019-210), no auto-cleanup; stale `--permission-mode` flag | Provider-transport conformance suite incl. **Codex-hang watchdog + auto-cleanup** (orchestrator parent reaps a hung child within a bounded deadline) and **removal of the stale `--permission-mode` flag** |
| **G4** Docker / no-network / bundle-cache | GOLD_SUITE bundle-cache + MarkupSafe/Cookiecutter NUL-dependency repair (Runs 330-332, 2026-06-14) | Docker + no-network + bundle-cache smoke passes **and** a NUL-dependency repair smoke over the GOLD_SUITE (MarkupSafe / Cookiecutter) reproduces the landed fix |
| **G5** Finalization / drain + supervisor cleanup + throughput | run48 killed at 8h27m due to A3-cap throughput collapse, only **3/15 finalized**, repos stuck live-1.0-unfinalized | A drain/finalization + supervisor-cleanup smoke finalizes all started rollouts within a throughput bound; **no live-1.0 stuck state**; supervisor cleans up orphaned children |
| **G6** Workspace / artifact reconciliation | statsmodels run **0.0%** collapse: missing workspaces (`No such file or directory: .../workspaces/rollout_19`), 17668 expected tests missing, no orchestrator stage | Reconciliation smoke asserts **every `rollout_id` has a materialized workspace + artifact, or a classified `ENV_*` abstain — never a silent 0.0%** (collapse must surface as a classified failure, not a silent zero) |
| **G7** Kill-mid-run → resume → same accepted artifact | (forward-looking; the resume mechanism is built in §22.2.3) | **REFERENCE §15.5** — do not duplicate that spec. G7 *promotes* the §15.5 kill→resume→same-accepted-artifact smoke to a **stabilization exit criterion**: kill mid-run, resume, and assert the identical accepted artifact bytes |
| **G8** Close the PHASE7 gate matrix | PHASE7: 3 PASS / 1 PASS-schema-only / **2 SKIPPED-NOT-EXERCISED** / **1 FAIL** (gate 5; all 8 carry "TODO: file upstream issue") | Turn **gate-5 FAIL → PASS**; **exercise the 2 skipped-not-exercised gates end-to-end** (fairness-delta and mutation-kill), so the matrix is fully exercised, not schema-only |
| **G9** Fix dead production wiring; block focal-symbol fabrication | V4 #2/#3: C3-defer / `mock_path` / `attribute_chain` validators **0/160 activated** (dead code despite 62 eligible); V4 #4: focal-symbol fabrication detected by tier-1 but `record_and_continue` ships it | Wire C3-defer / `mock_path` / `attribute_chain` so they run in the production path (assert non-zero activation on a slice where they are eligible) **and BLOCK** focal-symbol fabrication (tier-1 `fail` becomes blocking, not record-and-continue) — *before* relying on those audit fixes downstream |

G9 is sequenced deliberately: the V4 audit fixes are *dead in production* today (0/160 activated), so any later phase that "relies on the audit" is relying on code that never runs. G9 makes the audit real before Phase 0 trusts it. G2 is the gate that makes local validation *eligible to be evidence at all* — it is upstream of every later use of local signal as a training or acceptance input, consistent with §20.5's Youden gate (`J > 0` strict) and §13's reliability-weighted critic (`critic.min_reliability`).

#### 24.6.3 Executable benchmark-fairness CI gates (F1-F5)

Required-change #8 demands the `BENCHMARK_FAIRNESS_CHECKLIST.md` rules be **executable**, not prose. F1-F5 are CI gates wired to the verified module `apex/core/fairness_audit.py`, whose real surface is:

- `class FairnessAuditMode(str, Enum)` with members `OFF` / `PARALLEL` / `UPSTREAM_ONLY` (lines 51-73).
- `@runtime_checkable class BenchmarkScorerProtocol(Protocol)` with `score_task(self, task, apex_artifacts) -> dict[str, Any]` (lines 182-215).
- `def run_fairness_audit(task, apex_artifacts, private_scorer, upstream_scorer, *, shared_image_digest=None, extra_notes=None) -> FairnessDelta` (lines 299-355).
- `FLAG_THRESHOLD = 0.02` (line 368); a task is flagged when any numeric `abs(delta) > FLAG_THRESHOLD`.
- `@dataclass FairnessDelta` with the audit-anchor field `shared_image_digest: str | None` (line 130), plus `task_id`, `apex_private_scores`, `upstream_canonical_scores`, `delta`, `notes`.
- `class FairnessAuditAggregator` with `add_task` / `summary` / `write_to(path)` emitting `fairness_audit.json` + `FAIRNESS_REPORT.md`.

| Gate | Checklist clause (executable form) | Assertion against the module / manifest |
|------|------------------------------------|-----------------------------------------|
| **F1** Hidden-metadata scan | "Contamination and Leakage Checks"; "Hidden benchmark metadata stayed in the harness layer" | Scan agent workspace + prompt + orchestrator control state for gold/`test_patch`/`FAIL_TO_PASS`/`PASS_TO_PASS`/hidden test-id fields; assert **none present** in published-parity runs |
| **F2** pass@1-vs-best-of-N separation | "Pass@1 results from best-of-N or reranked results"; "state exactly how one final submission was chosen" | Assert the run manifest records the sampling mode and the selection rule that produced the single submission; **fail if a best-of-N/reranked number is reported as pass@1** |
| **F3** Leakage / scrubber assertions | "Scrub persisted artifacts…"; "Persisted artifacts do not leak hidden test identifiers" | Assert §4.2.7 scrub-at-load held: `SWEBenchTask` carries no `patch`/`test_patch` field; Commit0 git-history flatten post-condition `rev-list --all --count == 1`; persisted artifacts contain no hidden test ids |
| **F4** Network / memory / tool reporting | "Required Ablation Flags"; "Network policy matches the reported mode" | Assert manifest records network policy, repo-memory enabled/disabled, rollout count + selection policy, search mode/budget, delegated multi-agent mode — and that the **executed** network policy matches the **reported** one |
| **F5** Orchestrator-grade vs benchmark-grade separation | "Orchestrator success from benchmark grade success"; "upstream-canonical number is the publishable number" | Run `run_fairness_audit(...)` with a `private_scorer` and `upstream_scorer` sharing `shared_image_digest`; emit `FairnessDelta`; **upstream-canonical is the headline**, the APEX-private number is diagnostic-only, and the per-metric delta is published (flagged when `abs(delta) > FLAG_THRESHOLD = 0.02`) |

**Honest caveat (do not overstate F-gate readiness).** Per the module docstring (lines 16-34), `fairness_audit.py` is Phase-0-of-its-own-build: it ships the data model, aggregator, and `run_fairness_audit` driver, but it **does not yet wire into the benchmark runners** (Commit0/TestGenEval/SWT-Bench). The concrete scorers — `Commit0PrivateScorer`, `Commit0UpstreamScorer`, `TestGenEvalPrivateScorer`, `TestGenEvalUpstreamScorer`, `SWTBenchUpstreamScorer` — and the runner wiring are explicitly **Phase-1 TODO**. Therefore F-gates are **partly a wiring task**, and we sequence them honestly:

- **F1-F4 ship in Phase -1 as artifact/manifest assertions.** They need no scorers — they assert on the run manifest, the persisted artifacts, and the workspace/prompt/control state. They are buildable today against `BENCHMARK_FAIRNESS_CHECKLIST.md` and §4.2.7.
- **F5 is gated to land with the scorer/runner wiring** and is **marked as such** in `stabilization_gates.json`. F5 requires the concrete `BenchmarkScorerProtocol` implementations and the shared-`shared_image_digest` enforcement that the docstring assigns to Phase 1. Until then, F5 status is `pending_phase1_wiring`, not `pass`. (SWT-Bench legitimately has one scorer path; `run_fairness_audit` may be called with `private_scorer is upstream_scorer`, yielding an all-zero delta — the correct, honest outcome, per lines 26-29.)

This sequencing is the executable enforcement of the §4.2.7 anti-cheat prose, not a parallel policy: F1-F5 enforce the *same* "upstream Docker harness is the only publishable number; APEX-private is a delta at `FLAG_THRESHOLD = 0.02`" rule that §4.2.7 already states, and the `BENCHMARK_FAIRNESS_CHECKLIST.md` Review Gate conditions become CI assertions rather than reviewer prose.

#### 24.6.4 The `stabilization_gates.json` artifact

Each gate records per-gate status + an evidence pointer so Phase 0 entry is auditable. Schema:

```python
@dataclass(frozen=True)
class StabilizationGateRecord:
    gate_id: str            # "G1".."G9" | "F1".."F5"
    title: str              # human-readable gate name
    status: str             # "pass" | "fail" | "pending_phase1_wiring"
    verified_failure: str   # the V4/GOLD_SUITE/PHASE7/lastfailed failure it closes
    evidence_pointer: str   # path|test-id|log-line, e.g. "PHASE7_VALIDATION_REPORT.md:164"
    ci_job: str             # CI job name that produces this status
    checked_at_commit: str  # orchestrator git SHA at evaluation
    notes: str | None       # e.g. "F5 deferred: scorers are Phase-1 TODO per module docstring"
```

The file is `{run_dir}/stabilization_gates.json`, a top-level object `{"gates": [StabilizationGateRecord, ...], "all_green": bool, "phase0_unblocked": bool}`. `phase0_unblocked` is `true` iff **all nine G-gates and F1-F4 are `pass`** (F5 may be `pending_phase1_wiring` without blocking Phase 0, because its dependency is Phase-1 scorer wiring, but it must be `pass` before any *published* run — see the precision rule below).

#### 24.6.5 Exit criterion and the publish precision rule

**Phase -1 exit criterion.** All nine gates (G1-G9) and **F1-F4** are green on the pinned slice before Phase 0 (§22.2) begins, with per-gate status + evidence pointer recorded in `stabilization_gates.json` (`phase0_unblocked == true`). This makes required-change #3's "pre-implementation stabilization phase" literal and gated, and it makes the §22.1 sequencing rule honest: the engine/Executor/journal critical path is not started on an unstable kernel.

**Publish precision rule (executable §4.2.7).** A run is **not publishable** until **F1-F5 all pass** AND the `BENCHMARK_FAIRNESS_CHECKLIST.md` **Review Gate** conditions are recorded in the manifest:

> the manifest records the benchmark policy contract; the report states exact metric semantics; agent visibility mode is explicit; evaluator backend is explicit; memory usage is explicit; any non-pass@1 sampling/reranking is explicit; hidden benchmark metadata stayed in the harness layer.

These are asserted by F1-F4 (manifest/artifact/leakage/reporting) plus F5 (orchestrator-grade vs benchmark-grade separation via `FairnessDelta`). This is executable enforcement of the §4.2.7 anti-cheat prose — one policy, mechanized — not a second, parallel policy. The distinction between the two thresholds is deliberate: **Phase 0 may begin once the substrate is stable (G1-G9 + F1-F4)**, but **a number may only be published once the full fairness contract including F5 is mechanically satisfied** — which, per the module docstring, cannot be true until the Phase-1 scorer/runner wiring lands.

#### 24.6.6 Relationship to the roadmap and the narrowed thesis

Phase -1 inserts cleanly *before* §22.2 (Phase 0) and changes no Phase-0/1/2/3 content; it only adds a precondition. It also protects the narrowed novelty thesis: APEX-Ω's single defensible contribution is **no-retrain, held-out-vendor search/route-policy generalization under execution-authoritative, repo-level SWE patch acceptance with cost-matched budgets**. That thesis is only credible if (a) the execution-authoritative acceptance bar is real and not silently collapsing (G6), (b) the local validation feeding any learned signal is trustworthy (G2, ties to §20.5 Youden gate), (c) the audit that protects the published number actually runs in production (G9), and (d) the published number is mechanically fair (F1-F5). Without Phase -1, the review's "if only APEX v1 stabilization works → engineering report, not a splash paper" fallback (required-change #10) is the likely outcome by default; with it, the substrate is honestly green before the research bet is placed.

### 24.7 Durable Workflow-Graph Journal, Exact-vs-Artifact Replay, Restated Verification Invariant, CTDG-as-Telemetry, and Claude Attribution + Leaf Containment

This subsection bundles the five remaining required changes that share a common spine: each one replaces an over-claimed or under-specified mechanism in the original plan with a buildable spec grounded in the verified v1 source and the verified Anthropic docs. It (1) gives the full durable workflow-graph journal schema (required-change #5), framed as an **upgrade** of v1's existing escrow WAL + transcript rehydration rather than new-from-scratch durability; (2) splits replay into two products with a tightened kill/resume CI smoke (finding #10); (3) **deletes** the false ranking-tuple claim and restates the verification invariant as a two-part contract with property tests (finding #8); (4) pins CTDG's build order as telemetry-first, never an acceptance gate (required-change #9); and (5) corrects the Claude attribution and adds five leaf-containment rules (finding #5 / required-change #6). Each part is normatively scoped so a coding agent can act on it without re-reading the body sections.

#### 24.7.1 Journal Schema — Upgrade Escrow WAL + Transcript Rehydration into a Node-Level Graph Journal (required-change #5)

The original §15.3 already journals per-`agent()`-call records, but the schema there (`JournalEntry`) is a flat input-hash cache, not a **DAG-edged workflow-graph journal** keyed on `(agent, stage, item)` nodes. Verification confirms v1 has the durability *primitives* to build on — durable JSONL transcript rehydration on restart (`orchestrator_in_container_agent.py:1138`) and a flock-guarded, fsync-durable, monotonic-seq, idempotent candidate-promotion escrow WAL (`rollout/engine.py:31989`, `persistence/escrow.py`) — but **not** a graph journal that resumes incomplete nodes. So this is explicitly framed as *upgrading the existing escrow/transcript durability into a node-level workflow journal*, **not** "adding durability from scratch." The escrow WAL already solved the hard part (replay-stable ordering on monotonic `seq` rather than wall-clock, with idempotent keys so duplicate appends are harmless); we widen its key from one confirmed-pass candidate to every `(node_id, stage, item_id)` node and add DAG edges.

The authoritative record is the table below. §2.5's condensed durable-execution view and §15.3.1's `JournalEntry` both **defer to this table** as the single source of truth; where they disagree, this schema wins.

| Field | Type | Semantics |
|---|---|---|
| `run_id` | `str` | Run identity; root of the journal namespace. |
| `node_id` | `str` | Stable id of this `(agent, stage, item)` workflow node. |
| `parent_node_ids` | `list[str]` | **DAG edges** — upstream nodes whose outputs scope this node; resume re-runs a node only when all parents are terminal. |
| `attempt` | `int` | Re-execution counter for the same `node_id` (drives `idempotency_key`). |
| `agent_label` | `str` | Logical agent role (reproducer/localizer/patcher/test-writer/verifier/selector). |
| `stage` | `str` | Pipeline stage name (§9 stage identity). |
| `item_id` | `str` | The work item (task/candidate/test target) this node operates on. |
| `input_hash` | `str` (sha256) | Cache key over the canonical inputs (see §24.7.3 cache-validity). |
| `prompt_hash` | `str` (sha256) | Hash of `prompt_canonical`; **shared cross-reference key** between exact and artifact replay (§24.7.2). |
| `profile_hash` | `str` (sha256) | Hash of the resolved capability/cost profile vector used for routing (never one-hot vendor identity). |
| `scoped_inputs_hash` | `str` (sha256) | Hash of base snapshot SHA + discovery scope (file_paths/symbols/test_ids) + schema + effort. |
| `vendor` | `str` | `claude_cli` \| `codex_cli` \| `gemini_cli` \| `opencode_cli` \| `metacode_cli` \| `openai_api`. |
| `model_alias` | `str` | Logical alias requested (e.g. `opus`). |
| `resolved_model_id` | `str` | Pinned launcher id (e.g. `claude-opus-4-8[1m]`). |
| `cli_version` | `str` | Vendor CLI version (from `RunManifest.vendor_cli_versions`). |
| `tool_permissions` | `list[str]` | Allowlist/permission set the leaf ran under (audit + Claude-containment, §24.7.5). |
| `output_artifact_paths` | `list[str]` | Content-addressed paths of produced artifacts. |
| `diff_pointer` | `str` | Content-addressed blob ref for the produced git diff (the artifact-replay rehydration source). |
| `verifier_status` | `str` | `accepted` \| `rejected` \| `abstain` \| `inconclusive` \| `not_run`. |
| `terminal_state` | `str` | `ok` \| `infra_nonresult` \| `abstain` \| `failed`; resume re-runs any non-terminal node. |
| `usage` | `dict` | tokens in/out, cache_read/cache_creation. |
| `cost_usd` | `float` | Resolved spend for this node (§16 cost SLO). |
| `latency_ms` | `int` | Wall-clock latency, **journaled as data only — never read by orchestration** (the no-clock-in-orchestration invariant, §15.2). |
| `seq` | `int` | **Monotonic, flock-guarded order key** (`max(existing)+1`); the replay order, never wall-clock. |
| `idempotency_key` | `str` | `f"{run_id}::{node_id}::{attempt}"`; makes at-least-once external effects safe. |

The journal lives at `<run_dir>/journal/graph_wal.jsonl`, appended with the CCEDF discipline (fsync-durable, flock-guarded). The in-memory `next_seq` counter is recovered once from the WAL tail on startup so appends stay O(1). `latency_ms`, `cost_usd`, and `usage` are recorded for telemetry/SLO accounting but the orchestration (workflow) layer must never branch on them — branching on latency would re-introduce wall-clock nondeterminism and break replay.

#### 24.7.2 Two Replay Products — Exact Replay (kept) and Artifact Replay (additive) (finding #10)

Verification confirms v1 replay is **exact** prompt/tool-call substitution: `ReplayRecorder` monkey-patches the LLM CLI client and ACI tool executor so every `(prompt -> response)` and `(tool_invocation -> tool_result)` pair is journaled, and `ReplayPlayer` substitutes the recorded responses for live calls (`replay/__init__.py:5-26`). The plan's "artifact replay" (reapply diffs + rerun verification) is a **different** product. Replacing one with the other loses debugging coverage, so they ship as two separate, co-existing products:

| Property | Exact replay (kept UNCHANGED) | Artifact replay (ADDITIVE) |
|---|---|---|
| Source of truth | recorded `(prompt->response)` + `(tool->result)` JSONL | `diff_pointer` blob + re-run verification |
| Purpose | offline debugging (parser/prompt/model-output reproduction) | resume / audit (does the accepted artifact still hold) |
| `verify` | `verify=True` against recorded outputs | re-executes the verifier cascade against rehydrated diffs |
| Wiring | offline `apex replay` CLI; v1's `ReplayRecorder`/`Player` | wired into `agent()` via the `resume_or_run` wrapper (§15.3.2) |
| Cross-reference | shares `prompt_hash` with artifact replay | shares `prompt_hash` with exact replay |
| Status | preserved verbatim; the debugging workhorse | new second mode; **does not replace** exact replay |

The shared `prompt_hash` (schema field above) is the join key: a debugging session can pivot from an artifact-replay node to its exact-replay record and back. Artifact replay is therefore strictly additive — exact replay's debugging coverage is never sacrificed to gain resume.

Kill/resume CI smoke (finding #10). §15.5's `test_kill_midrun_identical_resume` is **reworded** so its assertions match the required contract exactly, gating artifact replay:

```
test_kill_midrun_identical_resume:   # gates artifact replay
  1. Start a fixed-seed run on a pinned task (RunManifest captured).
  2. SIGKILL the engine at a deterministic checkpoint (after node seq==K).
  3. Restart from the same run_dir.
  4. ASSERT same TERMINAL DISPATCH SET:
       the set of (node_id, stage, item_id) tuples that reached terminal_state
       is IDENTICAL across the killed run and the resumed run.
  5. ASSERT same ACCEPTED ARTIFACT VERDICT:
       same winner content-sha
       AND same re-run verifier_status / accept decision
       AND same diff text.
  6. ASSERT (negative): editing one node's prompt before restart re-runs EXACTLY
       that node and its dependents (via parent_node_ids), and nothing else.
```

The smoke asserts on the **terminal dispatch set** and the **accepted artifact verdict**, not token streams — consistent with §15.2's honesty boundary (we reproduce artifacts, not hosted-API token streams). **Until both replay products are green, durable resume is treated as unbuilt** (the v1 ingest note that `ReplayRecorder` has no production callsite still holds for the resume claim).

#### 24.7.3 Restated Verification Invariant — Delete the False Tuple Claim, State the Two-Part Contract (finding #8)

DELETE the original §15.6 / §13 claim that "the deterministic ranking tuple places every soft key strictly below every execution key." Source audit shows this is **factually wrong**: in `_best_cluster_by_deterministic_ranking` the tuple's **leading** sort key is `combined_score` (`selector.py:2942-2961`), with `accepted` 2nd and `verification_score` 6th — and `combined_score` is a **soft blend** of public, critic, process, evidence, consensus, size, and adversarial signals (`selector.py:202-304`). The safety property is **not** the tuple order. Replace the deleted claim with the two-part contract, each part grounded in the verified source:

1. **ACCEPTANCE GATE.** `verification.accepted` is computed **only** by the execution-grounded `_verification_meets_acceptance_bar` (`selector.py:1781/3537/5474/5511`). All learned/LLM/adversarial reviewers in `_apply_evidence_bound_review` (`selector.py:3682-3771`) are **downgrade-only** — every write to `verification.accepted` inside them is `= False` (`selector.py:3695/3723/3749`) — and they fail open. No code path sets `accepted=True` outside the bar.
2. **RANKING SCOPE.** Execution-acceptance filtering happens **upstream** via `_prefer_accepted_clusters` (`selector.py:7029-7030`, `return accepted or policy_valid`). The candidate set is restricted to execution-accepted clusters *before* ranking, so the `combined_score`-led tuple only orders candidates **within the already-accepted set** (and ranks unaccepted clusters only in the fallback where zero accepted clusters exist).

Plain-language invariant (state this, not the tuple mechanism): *learned/LLM signals may allocate budget, rank unaccepted candidates for more work, or break ties among already-execution-valid candidates, but can never convert an execution-invalid candidate into an accepted patch.*

Three property tests + one trace assertion (all mandatory CI gates):

| Test | Spec | What it proves |
|---|---|---|
| **PT1** | Inject an arbitrarily high `combined_score` (and high critic/public signal) onto an *unaccepted* cluster while an accepted cluster exists; assert the accepted cluster is selected. | A high soft score cannot win selection over an execution-accepted candidate. |
| **PT2** | Enumerate every reviewer hook in `_apply_evidence_bound_review`; assert each can only flip `accepted` True->False, and assert no code path sets `accepted=True` outside `_verification_meets_acceptance_bar`. | Reviewers are downgrade-only; the bar is the sole acceptance authority. |
| **PT3** | Construct a pool with zero accepted clusters; assert the high-`combined_score` unaccepted cluster, even if ranked first, is **not promoted to accepted** (selection may return it for *more work*, never as an accepted patch). | Ranking among unaccepted candidates allocates budget; it does not grant acceptance. |
| **Trace assertion** | Assert `_best_cluster_by_deterministic_ranking` is only ever called on the output of `_prefer_accepted_clusters` / `_build_selector_candidate_pool` (`selector.py:2273/2351/2381`), never on raw `clusters`. | The accepted-filter always runs before ranking; the tuple never sees unfiltered candidates. |

#### 24.7.4 CTDG Build Order — Telemetry First, Acceptance Gate Never (required-change #9)

CTDG (§10) ships **first as telemetry**, not as a gate. The first milestone delivers only observational/advisory outputs:

- **Telemetry:** coverage maps, time-to-first-failure, branch-prioritization signal, and the "pruned-but-would-have-passed" / "downgraded-but-won" canary metrics from §10.4.2.
- **Test prioritization** (the PRE channel, §10.4.1) **with a mandatory full-suite backstop** — reorder tests, never exclude them; the full suite always runs at the final pre-accept state.
- **The dynamic-prune channel** (POST, §10.4.3 `prune-with-backstop` / `prune-hard`) is a **later, per-repo-gated milestone**, enabled only after a measured non-100% catch rate is accepted for that repo. It is **never on the acceptance path**.

Re-anchor §10.1's tagline. The line "the CTDG feeds priors; the verifier decides" must be re-anchored to the **corrected** invariant of §24.7.3 — i.e. the verifier's authority is the *accepted-filter (`_prefer_accepted_clusters`) plus the execution acceptance bar (`_verification_meets_acceptance_bar`)*, **not** the deleted "soft-keys-below-execution-keys tuple." CTDG priors may re-weight branch budget and test order; they can never enter the acceptance gate. The §22 roadmap places the CTDG dynamic-prune channel **after the v1 stabilization slice** (the stabilization gates of required-change #3), so prune-with-backstop only lands once the substrate is trustworthy.

#### 24.7.5 Claude Attribution Fix + Five Leaf-Containment Rules (finding #5 / required-change #6)

Attribution fix. Verification against the primary Anthropic doc ([code.claude.com/docs/en/workflows](https://code.claude.com/docs/en/workflows)) confirms the exact documented semantics: a Claude workflow is *only* "a JavaScript script that orchestrates subagents"; intermediate state lives in **script variables** (not the conversation); subagents have **separate contexts**; convergence is via independent **adversarial verify/refute**; resume is **same-session cached** but a full Claude-Code exit "starts the workflow fresh"; and the script has **no direct filesystem or shell access** (agents read/write/run commands through allowed tools). These are the only guarantees we may cite. The following are **APEX extensions, not documented Claude guarantees**, and every comparison-matrix row must read "APEX extension": the five primitives (`agent`/`parallel`/`pipeline`/`phase`/`budget`), deterministic journaled replay (the `Date.now`/`Math.random` ban is imported from the durable-execution/Temporal literature, not from Claude), the WAL, and cross-vendor leaf execution. Reserve the term **"ultracode"** strictly for the Claude-Code setting it names (xhigh reasoning effort + automatic workflow orchestration); it is never a synonym for the APEX engine. The Executive Summary and intro must stop branding the paradigm "the ultracode model."

Five leaf-containment rules (all documented as feasible). When Claude Code runs as an APEX leaf worker:

| Rule | Mechanism | Why |
|---|---|---|
| **R1 — disable native workflows** | `disableWorkflows: true` / `CLAUDE_CODE_DISABLE_WORKFLOWS=1` / managed settings on every Claude leaf, except one labeled reference-workflow experiment. | Nested, unjournaled Claude orchestration must not escape APEX's scheduler/budget/replay. |
| **R2 — no ultracode trigger** | Never pass `/effort ultracode`; disable the Ultracode keyword trigger in config; scrub `ultracode`/`workflow`/"use a workflow" phrasing from leaf prompts. | Prevents accidental nested workflow spawns from prompt phrasing or the keyword trigger. |
| **R3 — noninteractive-safe** | Use `claude -p` / the Agent SDK (which never prompt), or pre-populate the tool allowlist with every shell/web/MCP command the leaf needs. | Subagents inherit the allowlist and run `acceptEdits`; an unallowlisted tool would otherwise **hang** a non-interactive run. |
| **R4 — log version + model** | Record the exact Claude Code version and resolved model id (journal fields `cli_version` + `resolved_model_id`, §24.7.1). | Replay/audit soundness across a heterogeneous fleet; a CLI-version mismatch correctly re-runs the node. |
| **R5 — checkpoints between runs** | Human judgment is a checkpoint boundary **between** workflow runs, never mid-run; run each stage as its own workflow. | Native workflows take no mid-run user input ("the next session starts fresh"); mid-run intervention is not a supported operation. |

These five rules make Claude-as-leaf fully contained within APEX's journaled scheduler while staying inside the documented Anthropic semantics, and they are the operational complement to the attribution fix above.
